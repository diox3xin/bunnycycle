...
// ========================
// ПЛАВАЮЩАЯ МЕНЮШКА ЗАЙЧИКА
// ========================
function initFloatingWidget() {
    // Создаём floating button
    const fab = document.createElement('div');
    fab.id = 'bc-fab';
    fab.innerHTML = '🐰';
    fab.title = 'BunnyCycle — быстрый обзор';
    document.body.appendChild(fab);

    // Создаём popup-панель
    const panel = document.createElement('div');
    panel.id = 'bc-fab-panel';
...
function toggleFabPanel() {
    const panel = document.getElementById('bc-fab-panel');
    if (!panel) return;

    if (panel.style.display === 'none') {
        renderFabPanel();
        panel.style.display = 'block';
        // Позиционируем рядом с fab
        const fab = document.getElementById('bc-fab');
        const fabRect = fab.getBoundingClientRect();
        panel.style.left = Math.max(8, fabRect.left - 280) + 'px';
        panel.style.top = Math.max(8, fabRect.top - 320) + 'px';
    } else {
        panel.style.display = 'none';
    }
}
...
function renderFabPanel() {
    ...
    if (!chars.length) {
        panel.innerHTML = '<div class="bc-fab-content"><div class="bc-fab-header">🐰 BunnyCycle</div><div class="bc-empty">Нет персонажей</div></div>';
        return;
    }

    let html = '<div class="bc-fab-content">';
    html += `<div class="bc-fab-header">🐰 BunnyCycle <span class="bc-fab-date">${formatDateShort(s.worldDate)}</span></div>`;
    ...
    for (const name of chars) {
        ...
        if (p.pregnancy?.active) {
            const w = p.pregnancy.week || 0;
            status.push(`🤰 ${w} нед.`);
        }
        if (p.labor?.active) status.push('🏥 Роды!');
        if (p.heat?.active) status.push('🔥 Течка');
        if (p.rut?.active) status.push('🔥 Гон');
        if (p.cycle?.enabled && !p.pregnancy?.active) {
            const day = p.cycle.currentDay || 0;
            const len = p.cycle.length || 28;
            status.push(`📅 д.${day}/${len}`);
        }
        if (p.health?.conditions?.length) {
            status.push(`🤒 ${p.health.conditions.length} бол.`);
        }
        if (p.health?.injuries?.length) {
            status.push(`🩹 ${p.health.injuries.length} тр.`);
        }
        if (p.mood?.current && p.mood.current !== 'neutral') {
            const moodEmoji = { happy:'😊', sad:'😢', angry:'😠', scared:'😨', aroused:'🥵', exhausted:'😩', in_pain:'🤕' };
            status.push(moodEmoji[p.mood.current] || '');
        }
        
        html += `<div class="bc-fab-char">`;
        html += `<span class="bc-fab-name">${name} ${sexIcon}</span>`;
        html += `<span class="bc-fab-status">${status.join(' · ') || '—'}</span>`;
        html += `</div>`;
    }
...
function formatDateShort(wd) {
    if (!wd) return '—';
    return `${wd.day || 1}.${String(wd.month || 1).padStart(2,'0')}.${wd.year || 2025}`;
}
