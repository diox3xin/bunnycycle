// ============================================================
// LifeCycle Extension v0.8.1 — index.js (CLEAN REWRITE)
// All features from v0.8.0, clean HTML, no syntax errors
// ============================================================
...
function roll(n) { return Math.floor(Math.random() * (n || 100)) + 1; }
...
const Profiles = {
    id() {
        ...
        if (!ctx) return null;
        if (ctx.groupId) return "grp_" + ctx.groupId;
        if (ctx.characterId !== undefined && ctx.characters) {
            const c = ctx.characters[ctx.characterId];
            if (c) return "chr_" + c.avatar + "_" + (ctx.chatId || "0");
        }
        return null;
    },
    ...
    load() {
        ...
        if (s.chatProfiles?.[cid]) {
            const pr = s.chatProfiles[cid];
            s.characters = JSON.parse(JSON.stringify(pr.characters || {}));
            s.relationships = JSON.parse(JSON.stringify(pr.relationships || []));
            s.worldDate = { ...(pr.worldDate || DEFAULTS.worldDate) };
            s.diceLog = [...(pr.diceLog || [])];
            s.intimacyLog = [...(pr.intimacyLog || [])];
        } else {
            s.characters = {};
            s.relationships = [];
            s.diceLog = [];
    ...
    del(id) {
        const s = S();
        if (s.chatProfiles?.[id]) { delete s.chatProfiles[id]; saveSettingsDebounced(); }
    },
};
...
const REL_TYPES = [
    ...
    "дедушка", "бабушка", "внук", "внучка", "друг", "возлюбленный(ая)", "другое",
];

const Rels = {
    get() { return S().relationships || []; },

    add(c1, c2, type, notes) {
        const s = S();
        if (!s.relationships) s.relationships = [];
        if (s.relationships.find(r => r.char1 === c1 && r.char2 === c2 && r.type === type)) return;
        s.relationships.push({ id: uid(), char1: c1, char2: c2, type, notes: notes || "" });
        saveSettingsDebounced();
    },
    ...
    addBirth(mo, fa, baby) {
        if (mo) { this.add(mo, baby, "мать", ""); this.add(baby, mo, "ребёнок", ""); }
        if (fa && fa !== "?") { this.add(fa, baby, "отец", ""); this.add(baby, fa, "ребёнок", ""); }
    },
...
// ==========================================
// 5. CHAR INFO PARSER (from card description)
// ==========================================
...
const CardParser = {
    parse(obj) {
        if (!obj) return {};
        const texts = [
            obj.description, obj.personality, obj.scenario, obj.first_mes,
            obj.data?.description, obj.data?.personality,
            obj.data?.extensions?.depth_prompt?.prompt,
        ].filter(Boolean);
        const t = texts.join("\n");
        const info = {};
        ...
        // Eyes & hair
        let m = t.match(/\b(голуб\S*|сер\S*|зелен\S*|кар\S*|чёрн\S*|янтарн\S*|золот\S*|фиолетов\S*|blue|green|brown|grey|amber|gold|red|violet)\s*(?:eye|eyes|глаз)/i);
...
const ChatParser = {
    ...
    parse(msgs, chars) {
        ...
        for (const name of names) {
            ...
            // Bio sex
            let fS = 0, mS = 0;
            if (/\b(?:female|woman|девушка|женщина)\b/i.test(nearText)) fS += 10;
            if (/\b(?:male|man|мужчина|парень)\b/i.test(nearText)) mS += 10;
            if (/(?:её|her)\s*(?:грудь|живот|матк|breast|womb)/i.test(nearText)) fS += 8;
            if (/(?:его|his)\s*(?:член|cock|dick)/i.test(nearText)) mS += 8;
            fS += (nearText.match(/\b(она|её|ей|she|her)\b/gi) || []).length;
            mS += (nearText.match(/\b(он|его|ему|he|him)\b/gi) || []).length;
            if (fS > mS * 1.3 && fS >= 5) info.bioSex = "F";
            else if (mS > fS * 1.3 && mS >= 5) info.bioSex = "M";
            ...
            if (info.secondarySex || info.bioSex || info.events.length || info.children.length) {
                results[name] = info;
            }
        }
        return results;
    ...
    _state(events) {
        ...
        let lPreg = -1, lEnd = -1, lBirth = -1, lLabor = -1, lHeat = -1, lRut = -1, lOvi = -1;

        for (const e of events) {
            if (e.t === "preg") lPreg = e.i;
            if (e.t === "preg_end") lEnd = e.i;
            if (e.t === "birth") lBirth = e.i;
            if (e.t === "labor") lLabor = e.i;
            if (e.t === "heat") lHeat = e.i;
            if (e.t === "rut") lRut = e.i;
            if (e.t === "ovi") lOvi = e.i;
        }

        if (lPreg > lEnd && lPreg > lBirth) st.pregnant = true;
        if (lBirth > lPreg) { st.birthDone = true; st.pregnant = false; st.inLabor = false; }
        if (lEnd > lPreg) st.pregnant = false;
        if (lLabor > lBirth) st.inLabor = true;
        st.inHeat = lHeat > -1;
        st.inRut = lRut > -1;
        st.ovi = lOvi > -1;
        ...
        for (let i = 0; i < days; i++) {
            ...
            if (this.c.currentDay > this.c.length) {
                ...
                if (this.c.irregularity > 0) {
                    this.c.length = clamp(this.c.baseLength + Math.floor(Math.random() * this.c.irregularity * 2) - this.c.irregularity, 21, 45);
                }
            }
        }
    }
...
class HeatRutManager {
    ...
    hPhase() {
        const h = this.p.heat;
        if (!h) return "rest";
        if (h.active) { if (h.currentDay <= 1) return "preHeat"; if (h.currentDay <= h.duration - 1) return "heat"; return "postHeat"; }
    ...
    rPhase() {
        const r = this.p.rut;
        if (!r) return "rest";
        if (r.active) { if (r.currentDay <= 1) return "preRut"; if (r.currentDay <= r.duration - 1) return "rut"; return "postRut"; }
    ...
    advH(d) {
        ...
        for (let i = 0; i < d; i++) {
            ...
            else { h.daysSinceLast = (h.daysSinceLast || 0) + 1; if (h.daysSinceLast >= h.cycleDays) { h.active = true; h.currentDay = 1; h.intensity = "severe"; } }
        }
    }
    advR(d) {
        const r = this.p.rut;
...
class PregManager {
    ...
    start(father, count, sexes) {
        ...
        this.pr.complicationsEnabled = true;
...
class LaborManager {
    ...
    start(diff) {
        ...
        this.l.complicationsEnabled = true;
    }
...
class BabyManager {
    ...
    milestones() {
        ...
        if (d >= 90) r.push("голову");
        if (d >= 180) r.push("сидит");
        if (d >= 365) r.push("ходит");
...
class OviManager {
    ...
    startCarrying(count, father) {
        ...
        this.o.active = true; this.o.phase = "carrying";
        this.o.eggCount = c; this.o.gestationDay = 0;
        ...
        for (let i = 0; i < c; i++) {
            this.o.eggs.push({
                fertilized: Math.random() < (cfg.fertilizationChance || 0.7),
    ...
    promptText() {
        ...
        if (this.o.phase === "laying") return `LAYING EGGS Day ${this.o.layingDay}/${this.o.layingMax}. Eggs passing through birth canal. Shell: ${cfg.shellType}. ${this.symptoms().join(", ")}`;
        ...
        if (this.o.phase === "hatched") return `HATCHING! ${this.o.fertilizedCount} offspring emerging from eggs.`;
        return "";
    }
}
...
const Intimacy = {
    ...
    roll(tg, d) {
        ...
        if (d.nc) { /* ignore contraception */ } else if (d.co) f *= 0.15; else f *= (1 - ce);
...
const TimeParse = {
    ...
    apply(parsed) {
        ...
        if (parsed.setDate) {
            ...
            const diff = Math.round((t - c) / 864e5);
            if (diff > 0) da = diff;
...
const Prompt = {
    gen() {
        ...
        const d = s.promptInjectionDetail;
        const lines = ["[LifeCycle]", "Date: " + fmt(s.worldDate)];
        const rt = Rels.toPrompt();
        if (rt) lines.push("\n" + rt);
        ...
        Object.entries(s.characters).forEach(([n, p]) => {
            ...
            if (s.modules.pregnancy && p.pregnancy?.active) {
                ...
                lines.push("PREGNANT Wk" + p.pregnancy.week + "/" + p.pregnancy.maxWeeks + " T" + pm.tri() + ", " + pm.size() + ", Moves: " + pm.moves());
            ...
            if (s.auSettings.oviposition?.enabled && p.oviposition?.active) {
                lines.push(new OviManager(p).promptText());
            }

            if (s.modules.baby && p.babies?.length > 0 && d !== "low") {
                p.babies.forEach(b => lines.push("Child: " + (b.name || "?") + " (" + (b.sex === "M" ? "♂" : "♀") + ", " + new BabyManager(b).age() + ")"));
            }
...
function syncChars() {
    ...
    for (const c of active) {
        ...
        if (c.obj && s.autoParseCharInfo) {
            const parsed = CardParser.parse(c.obj);
        ...
        if (s.autoParseCharInfo && c.obj && !c.isUser) {
            ...
            if (parsed.bioSex && !pr._mB) { pr.bioSex = parsed.bioSex; if (parsed.bioSex === "M" && !pr._mCyc) pr.cycle.enabled = false; changed = true; }
            if (parsed.secondarySex && !pr._mS) { pr.secondarySex = parsed.secondarySex; if (parsed.secondarySex === "omega" && pr.bioSex === "M") pr.cycle.enabled = true; changed = true; }
            if (parsed.race && !pr._mR) { pr.race = parsed.race; changed = true; }
            if (parsed.eyeColor && !pr._mE) { pr.eyeColor = parsed.eyeColor; changed = true; }
            if (parsed.hairColor && !pr._mH) { pr.hairColor = parsed.hairColor; changed = true; }
            if (parsed.canLayEggs) { pr._canLayEggs = true; changed = true; }
        }
    }
    ...
    if (s.parseFullChat) {
        ...
        if (ctx?.chat?.length > 0) {
            const parsed = ChatParser.parse(ctx.chat, s.characters);
            for (const [name, info] of Object.entries(parsed)) {
                const p = s.characters[name];
                if (!p) continue;

                if (info.secondarySex && !p._mS) { p.secondarySex = info.secondarySex; changed = true; }
                if (info.bioSex && !p._mB) { p.bioSex = info.bioSex; if (info.bioSex === "M" && !p._mCyc) p.cycle.enabled = false; changed = true; }
...
// ==========================================
// 12. HTML GENERATION (CLEAN, ALL TAGS CLOSED)
// ==========================================
...
function genHTML() {
    const s = S();
    return `
<div id="lifecycle-panel" class="lifecycle-panel ${s.panelCollapsed ? 'collapsed' : ''}">
    <div class="lifecycle-header" id="lifecycle-header-toggle">
        <div class="lifecycle-header-title">
            ...
            <h3>🌿 LifeCycle</h3>
            <span class="lc-version">v0.8.1</span>
        </div>
    ...
    <div class="lifecycle-body">
        ...
        <div class="lifecycle-tabs">
            ...
            <button class="lifecycle-tab" data-tab="profiles"><span class="tab-icon">💾</span>Проф.</button>
        ...
        <div class="lifecycle-tab-content active" data-tab="chars">
            <div class="lc-btn-group" style="margin-bottom:6px">
                <button class="lc-btn lc-btn-primary lc-btn-sm" id="lc-sync">🔄 Синхр.</button>
                <button class="lc-btn lc-btn-sm" id="lc-add-m">➕ Вручную</button>
                <button class="lc-btn lc-btn-sm" id="lc-reparse">📖 Перечитать</button>
            </div>
            ...
            <div id="lc-char-editor" class="lc-editor hidden">
                ...
                <div class="lc-editor-grid">
                    <div class="lc-editor-field">
                        ...
                        <select id="lc-ed-bio" class="lc-select"><option value="F">F</option><option value="M">M</option></select>
                    </div>
                    <div class="lc-editor-field">
                        <label>Втор. пол</label>
                        <select id="lc-ed-sec" class="lc-select"><option value="">Нет</option><option value="alpha">Alpha</option><option value="beta">Beta</option><option value="omega">Omega</option></select>
                    </div>
                    <div class="lc-editor-field">
                        <label>Раса</label>
                        <input type="text" id="lc-ed-race" class="lc-input" placeholder="human">
                    </div>
                    <div class="lc-editor-field">
                        <label>Контрацепция</label>
                        <select id="lc-ed-contra" class="lc-select"><option value="none">Нет</option><option value="condom">Презерватив</option><option value="pill">Таблетки</option><option value="iud">ВМС</option><option value="withdrawal">ППА</option></select>
                    </div>
                    <div class="lc-editor-field">
                        <label>Глаза</label>
                        <input type="text" id="lc-ed-eyes" class="lc-input">
                    </div>
                    <div class="lc-editor-field">
                        <label>Волосы</label>
                        <input type="text" id="lc-ed-hair" class="lc-input">
                    </div>
                    <div class="lc-editor-field">
                        <label>Сложность берем.</label>
                        <select id="lc-ed-diff" class="lc-select"><option value="easy">Лёгкая</option><option value="normal">Обычная</option><option value="hard">Тяжёлая</option></select>
                    </div>
                    <div class="lc-editor-field">
                        <label class="lc-checkbox"><input type="checkbox" id="lc-ed-on"><span>Включён</span></label>
                    </div>
                    <div class="lc-editor-field">
                        <label class="lc-checkbox"><input type="checkbox" id="lc-ed-cyc"><span>Цикл вкл</span></label>
                    </div>
                    <div class="lc-editor-field">
                        <label>Длина цикла</label>
                        <input type="number" id="lc-ed-clen" class="lc-input" min="21" max="45" value="28">
                    </div>
                    <div class="lc-editor-field">
                        <label>Менстр. дней</label>
                        <input type="number" id="lc-ed-mdur" class="lc-input" min="2" max="10" value="5">
                    </div>
                    <div class="lc-editor-field">
                        <label>Нерегулярность</label>
                        <input type="number" id="lc-ed-irreg" class="lc-input" min="0" max="7" value="2">
                    </div>
                </div>
        ...
        <div class="lifecycle-tab-content" data-tab="rels">
            <div class="lc-section">
                <div class="lc-row">
                    <select id="lc-rel-c1" class="lc-select lc-char-select">${charOpts()}</select>
                    <select id="lc-rel-tp" class="lc-select">${relTypeOpts()}</select>
                    <select id="lc-rel-c2" class="lc-select lc-char-select">${charOpts()}</select>
                </div>
                <div class="lc-row">
                    <input type="text" id="lc-rel-n" class="lc-input" placeholder="Заметка (необяз.)">
                    <button class="lc-btn lc-btn-success lc-btn-sm" id="lc-rel-add">➕</button>
                </div>
            </div>
            <div id="lc-rel-list" class="lc-scroll"></div>
        </div>

        <!-- CYCLE TAB -->
        <div class="lifecycle-tab-content" data-tab="cycle">
            <div class="lc-row"><label>Персонаж</label><select id="lc-cyc-char" class="lc-select lc-char-select">${charOpts()}</select></div>
            <div id="lc-cyc-panel"></div>
            <div class="lc-row" style="margin-top:6px">
                <label>День цикла</label>
                <input type="number" id="lc-cyc-day" class="lc-input" style="width:60px" min="1">
                <button class="lc-btn lc-btn-sm" id="lc-cyc-setday">Уст.</button>
            </div>
        ...
        <div class="lifecycle-tab-content" data-tab="hr">
            <div class="lc-row"><label>Персонаж</label><select id="lc-hr-char" class="lc-select lc-char-select">${charOpts()}</select></div>
        ...
        <div class="lifecycle-tab-content" data-tab="intim">
            <div class="lc-section">
                ...
                <div class="lc-row">
                    <label>Тип</label>
                    <select id="lc-int-tp" class="lc-select"><option value="vaginal">Вагинальный</option><option value="anal">Анальный</option><option value="oral">Оральный</option></select>
                </div>
                <div class="lc-row">
                    <label>Эякуляция</label>
                    <select id="lc-int-ej" class="lc-select"><option value="inside">Внутрь</option><option value="outside">Снаружи</option><option value="unknown">Неизв.</option></select>
                </div>
                <div class="lc-btn-group">
                    <button class="lc-btn lc-btn-sm" id="lc-int-log">📝 Записать</button>
                    <button class="lc-btn lc-btn-primary lc-btn-sm" id="lc-int-roll">🎲 Бросок</button>
                </div>
            </div>
            <h4>Лог бросков</h4>
        ...
        <div class="lifecycle-tab-content" data-tab="preg">
            <div class="lc-row"><label>Персонаж</label><select id="lc-preg-char" class="lc-select lc-char-select">${charOpts()}</select></div>
            ...
            <div class="lc-btn-group" style="margin-top:6px">
                ...
                <button class="lc-btn lc-btn-danger lc-btn-sm" id="lc-preg-labor">→ Роды</button>
                <button class="lc-btn lc-btn-danger lc-btn-sm" id="lc-preg-end">Прервать</button>
            </div>
        </div>

        <!-- LABOR TAB -->
        <div class="lifecycle-tab-content" data-tab="labor">
            <div class="lc-row"><label>Персонаж</label><select id="lc-labor-char" class="lc-select lc-char-select">${charOpts()}</select></div>
            <div id="lc-labor-panel"></div>
            <div class="lc-btn-group" style="margin-top:6px">
                <button class="lc-btn lc-btn-sm" id="lc-labor-adv">→ Стадия</button>
                <button class="lc-btn lc-btn-success lc-btn-sm" id="lc-labor-deliver">👶 Родить</button>
                <button class="lc-btn lc-btn-danger lc-btn-sm" id="lc-labor-end">Завершить</button>
            </div>
        </div>
        ...
        <div class="lifecycle-tab-content" data-tab="baby">
            <div class="lc-row">
                ...
                <select id="lc-baby-par" class="lc-select lc-char-select">${charOpts()}</select>
                <button class="lc-btn lc-btn-success lc-btn-sm" id="lc-baby-create">➕ Создать</button>
            </div>
        ...
        <div class="lifecycle-tab-content" data-tab="ovi">
            <div class="lc-row"><label>Персонаж</label><select id="lc-ovi-char" class="lc-select lc-char-select">${charOpts()}</select></div>
            <div id="lc-ovi-panel"></div>
            <div class="lc-btn-group" style="margin-top:6px">
                <button class="lc-btn lc-btn-sm" id="lc-ovi-start">🥚 Начать вынашивание</button>
                <button class="lc-btn lc-btn-sm" id="lc-ovi-adv">→ Продвинуть</button>
                <button class="lc-btn lc-btn-danger lc-btn-sm" id="lc-ovi-end">Завершить</button>
            </div>
        </div>

        <!-- PROFILES TAB -->
        <div class="lifecycle-tab-content" data-tab="profiles">
            <div class="lc-info" id="lc-prof-cur"></div>
            <div class="lc-btn-group" style="margin-bottom:6px">
                <button class="lc-btn lc-btn-success lc-btn-sm" id="lc-prof-save">💾 Сохранить</button>
                <button class="lc-btn lc-btn-primary lc-btn-sm" id="lc-prof-reload">🔄 Перезагрузить</button>
            </div>
        ...
        <div class="lifecycle-tab-content" data-tab="settings">
            ...
            <div class="lc-section">
                ...
                <div class="lc-row"><label>Позиция</label><select id="lc-ppos" class="lc-select"><option value="authornote" ${s.promptInjectionPosition==='authornote'?'selected':''}>Author's Note</option><option value="system" ${s.promptInjectionPosition==='system'?'selected':''}>System</option></select></div>
            ...
            <div class="lc-section">
                <h4>AU Пресет</h4>
                <select id="lc-aup" class="lc-select" style="width:100%;margin-bottom:6px"><option value="realism" ${s.auPreset==='realism'?'selected':''}>Реализм</option><option value="omegaverse" ${s.auPreset==='omegaverse'?'selected':''}>Омегаверс</option><option value="fantasy" ${s.auPreset==='fantasy'?'selected':''}>Фэнтези</option></select>
            ...
            <div class="lc-section">
                <h4>Дата мира</h4>
                <div class="lc-row">
                    <input type="number" id="lc-dy" class="lc-input" style="width:60px" value="${s.worldDate.year}">
                    <input type="number" id="lc-dm" class="lc-input" style="width:40px" min="1" max="12" value="${s.worldDate.month}">
                    <input type="number" id="lc-dd" class="lc-input" style="width:40px" min="1" max="31" value="${s.worldDate.day}">
                    <input type="number" id="lc-dh" class="lc-input" style="width:40px" min="0" max="23" value="${s.worldDate.hour}">
                    <button class="lc-btn lc-btn-sm" id="lc-da">✓</button>
                </div>
                <div class="lc-btn-group">
            ...
            <div class="lc-btn-group">
                ...
                <button class="lc-btn lc-btn-danger lc-btn-sm" id="lc-rst">🗑️ Сброс</button>
            </div>
        </div>
    </div>
</div>`;
...
function updateSels() {
    ...
    document.querySelectorAll(".lc-char-select").forEach(sel => {
        ...
        if (names.includes(v)) sel.value = v;
    });
}

function renderDash() {
    const s = S();
    const de = document.getElementById("lc-dash-date");
    const ie = document.getElementById("lc-dash-items");
    if (!de || !ie) return;
    ...
    Object.entries(s.characters).forEach(([n, p]) => {
        ...
        if (s.modules.cycle && p.cycle?.enabled && !p.pregnancy?.active) {
            const cm = new CycleManager(p);
            tags.push(cm.emoji(cm.phase()) + cm.label(cm.phase()));
        }
        ...
        h += `<div class="lc-char-card">
            <div class="lc-char-card-header">
                ...
                <div class="lc-char-card-actions">
                    ...
                    <button class="lc-btn lc-btn-danger lc-btn-sm lc-del-char" data-char="${n}">✕</button>
                </div>
            </div>
            <div class="lc-char-card-info">${p.race || "human"} | ${p.contraception} ${p.eyeColor ? "| 👁" + p.eyeColor : ""} ${p.hairColor ? "| 💇" + p.hairColor : ""}</div>
        </div>`;
    });
    el.innerHTML = h || '<div class="lc-empty">Нет персонажей</div>';
}
...
function renderCycle() {
    ...
    let fc = "low";
    if (f >= 0.2) fc = "peak"; else if (f >= 0.1) fc = "high"; else if (f >= 0.05) fc = "med";

    // Calendar
    let cal = '<div class="lc-cycle-calendar">';
    for (let d = 1; d <= p.cycle.length; d++) {
        const m = p.cycle.menstruationDuration, ov = Math.round(p.cycle.length - 14);
        let cls = "lut";
        if (d <= m) cls = "mens"; else if (d < ov - 2) cls = "foll"; else if (d <= ov + 1) cls = "ovul";
        if (d === p.cycle.currentDay) cls += " today";
        cal += `<div class="lc-cal-day ${cls}">${d}</div>`;
    }
    cal += '</div>';
    ...
    el.innerHTML = cal + `
        <div class="lc-cycle-info">
            ...
            <div class="lc-info-row">Выделения: ${cm.discharge()}</div>
            ${sym.length > 0 ? '<div class="lc-info-row">Симптомы: ' + sym.join(", ") + '</div>' : ''}
        </div>`;
...
function renderHR() {
    ...
    if (!p || !s.modules.auOverlay || s.auPreset !== "omegaverse" || !p.secondarySex) {
        el.innerHTML = '<div class="lc-empty">AU не активен или нет втор. пола</div>'; return;
    }
    const hr = new HeatRutManager(p);
    let h = "";

    if (p.secondarySex === "omega") {
        const hph = HeatRutManager.HP[hr.hPhase()];
        h += `<div class="lc-section">
            <h4>🔥 Течка — ${hph}</h4>`;
        if (p.heat.active) {
            h += `<div class="lc-info-row">День ${p.heat.currentDay}/${p.heat.duration}</div>`;
        } else {
            h += `<div class="lc-info-row">До след.: ${hr.hLeft()} дн.</div>`;
        }
        const hs = hr.hSym();
        if (hs.length) h += `<div class="lc-info-row">Симптомы: ${hs.join(", ")}</div>`;
        h += `<div class="lc-btn-group" style="margin-top:4px">
            <button class="lc-btn lc-btn-sm" id="lc-hr-th">🔥 Запустить</button>
            <button class="lc-btn lc-btn-sm" id="lc-hr-sh">⏹ Остановить</button>
            <button class="lc-btn lc-btn-sm" id="lc-hr-su">${p.heat.onSuppressants ? '💊 ВКЛ' : '💊 ВЫКЛ'}</button>
        </div></div>`;
    }

    if (p.secondarySex === "alpha") {
        const rph = HeatRutManager.RP[hr.rPhase()];
        h += `<div class="lc-section">
            <h4>💢 Гон — ${rph}</h4>`;
        if (p.rut.active) {
            h += `<div class="lc-info-row">День ${p.rut.currentDay}/${p.rut.duration}</div>`;
        } else {
            h += `<div class="lc-info-row">До след.: ${hr.rLeft()} дн.</div>`;
        }
        const rs = hr.rSym();
        if (rs.length) h += `<div class="lc-info-row">Симптомы: ${rs.join(", ")}</div>`;
        h += `<div class="lc-btn-group" style="margin-top:4px">
            <button class="lc-btn lc-btn-sm" id="lc-hr-tr">💢 Запустить</button>
            <button class="lc-btn lc-btn-sm" id="lc-hr-sr">⏹ Остановить</button>
        </div></div>`;
    }
...
function renderPreg() {
    ...
    const pm = new PregManager(p), pr = p.pregnancy;
    const prog = Math.round((pr.week / pr.maxWeeks) * 100);
    el.innerHTML = `
        <div class="lc-preg-header">
            <span class="lc-preg-week">Неделя ${pr.week}/${pr.maxWeeks}</span>
            <span class="lc-preg-trim">T${pm.tri()}</span>
        </div>
        <div class="lc-sw-mini-progress"><div class="lc-sw-mini-fill preg" style="width:${prog}%"></div></div>
        <div class="lc-info-row">Размер: ~${pm.size()} · Плодов: ${pr.fetusCount} (${(pr.fetusSexes||[]).map(x=>x==="M"?"♂":"♀").join("")})</div>
        <div class="lc-info-row">Шевеления: ${pm.moves()} · +${pr.weightGain || 0}кг</div>
        <div class="lc-info-row">Симптомы: ${pm.symptoms().join(", ") || "нет"}</div>
        ${pr.complications?.length > 0 ? '<div class="lc-info-row lc-comp-list">Осложнения: ' + pr.complications.map(c => '<span class="lc-comp-tag">' + c + '</span>').join("") + '</div>' : ''}
...
function renderLabor() {
    ...
    if (!el || !sel) return;
    const p = s.characters[sel.value];
    if (!p?.labor?.active) { el.innerHTML = '<div class="lc-empty">Нет родов</div>'; return; }
    ...
    el.innerHTML = `
        ...
        <div class="lc-info-row">Раскрытие: ${p.labor.dilation}/10 см · Часов: ${p.labor.hoursElapsed}</div>
        <div class="lc-info-row">Рождено: ${p.labor.babiesDelivered}/${p.labor.totalBabies} · Сложность: ${p.labor.difficulty}</div>
        <div class="lc-labor-desc">${lm.desc()}</div>
        ${p.labor.complications?.length > 0 ? '<div class="lc-comp-list">' + p.labor.complications.map(c => '<span class="lc-comp-tag">' + c + '</span>').join("") + '</div>' : ''}`;
}
...
function renderBabies() {
    ...
    p.babies.forEach((b, i) => {
        ...
        h += `<div class="lc-baby-card">
            <div class="lc-baby-header">
                <span class="lc-baby-name">${b.name || "?"} ${b.sex === "M" ? "♂" : "♀"}${b.secondarySex ? " · " + b.secondarySex : ""}</span>
                <span class="lc-baby-sex">${bm.age()} · ${b.state}</span>
            </div>
            <div class="lc-baby-details">
                ${b.eyeColor ? "👁" + b.eyeColor + " " : ""}${b.hairColor ? "💇" + b.hairColor + " " : ""}· ${Math.round(b.currentWeight)}г
                ${ms.length > 0 ? "<br>Вехи: " + ms.join(", ") : ""}
            </div>
            <div class="lc-baby-actions">
                <button class="lc-btn lc-btn-sm lc-baby-edit" data-p="${sel.value}" data-i="${i}">✏️</button>
                <button class="lc-btn lc-btn-danger lc-btn-sm lc-baby-del" data-p="${sel.value}" data-i="${i}">✕</button>
            </div>
        </div>`;
    });
    el.innerHTML = h;
...
function renderOvi() {
    ...
    const om = new OviManager(p);
    const prog = om.progress();
    const sym = om.symptoms();

    let eggH = '<div class="lc-ovi-eggs-grid">';
    (p.oviposition.eggs || []).forEach((e, i) => {
        eggH += `<div class="lc-ovi-egg-card ${e.fertilized ? 'fertilized' : 'unfertilized'}">
            <span class="lc-ovi-egg-icon">🥚</span>
            <div class="lc-ovi-egg-info"><strong>#${i + 1}</strong> ${e.fertilized ? "✓ фертильное" : "✗ пустое"}<br>${e.size}см · ${OviManager.PHASES[p.oviposition.phase]}</div>
        </div>`;
    });
    eggH += '</div>';

    el.innerHTML = `
        <div class="lc-ovi-phase">🥚 ${OviManager.PHASES[p.oviposition.phase]}</div>
        <div class="lc-sw-mini-progress"><div class="lc-sw-mini-fill ovi" style="width:${prog}%"></div></div>
        <div class="lc-ovi-stat">Яиц: ${p.oviposition.eggCount} (${p.oviposition.fertilizedCount} фертильных)</div>
        ${sym.length > 0 ? '<div class="lc-ovi-stat">Симптомы: ' + sym.join(", ") + '</div>' : ''}
        ${eggH}`;
}

function renderRels() {
    const s = S(), el = document.getElementById("lc-rel-list");
    if (!el) return;
    const rels = s.relationships || [];
    if (!rels.length) { el.innerHTML = '<div class="lc-empty">Нет связей</div>'; return; }
    el.innerHTML = rels.map(r =>
        `<div class="lc-char-card">
            <span>${r.char1} → ${r.char2}: <strong>${r.type}</strong>${r.notes ? " (" + r.notes + ")" : ""}</span>
            <button class="lc-btn lc-btn-danger lc-btn-sm lc-del-rel" data-id="${r.id}">✕</button>
        </div>`
    ).join("");
...
function renderProfs() {
    const s = S();
    const curEl = document.getElementById("lc-prof-cur");
    if (curEl) curEl.textContent = "Текущий: " + (s.currentChatId || "—") + " (" + Object.keys(s.characters).length + " перс.)";
    const el = document.getElementById("lc-prof-list");
    if (!el) return;
    const list = Profiles.list();
    if (!list.length) { el.innerHTML = '<div class="lc-empty">Нет сохранённых профилей</div>'; return; }
    el.innerHTML = list.map(p =>
        `<div class="lc-profile-card ${p.isCurrent ? 'current' : ''}">
            <div class="lc-profile-card-info">
                <div class="lc-profile-card-name">${p.id}${p.isCurrent ? " ★" : ""}</div>
                <div>${p.count} перс. · ${p.date}</div>
            </div>
            <div class="lc-btn-group">
                <button class="lc-btn lc-btn-sm lc-prof-load" data-id="${p.id}">📂</button>
                <button class="lc-btn lc-btn-danger lc-btn-sm lc-prof-del" data-id="${p.id}">✕</button>
            </div>
        </div>`
    ).join("");
}
...
function renderDice() {
    const s = S(), el = document.getElementById("lc-dice-log");
    if (!el) return;
    if (!s.diceLog.length) { el.innerHTML = '<div class="lc-empty">Нет бросков</div>'; return; }
    el.innerHTML = [...s.diceLog].reverse().slice(0, 20).map(d =>
        `<div class="lc-dice-entry ${d.result ? 'lc-dice-success' : 'lc-dice-fail'}">
            <span class="lc-dice-ts">${d.ts}</span> ${d.target}: ${d.roll}/${d.chance}% → ${d.result ? "🤰 ЗАЧАТИЕ!" : "промах"}
        </div>`
    ).join("");
}

function renderIntim() {
    const s = S(), el = document.getElementById("lc-intim-log");
    if (!el) return;
    if (!s.intimacyLog.length) { el.innerHTML = '<div class="lc-empty">Пусто</div>'; return; }
    el.innerHTML = [...s.intimacyLog].reverse().slice(0, 20).map(e =>
        `<div class="lc-intim-entry"><span class="lc-intim-ts">${e.ts}</span> ${(e.parts || e.pa || []).join("+")} · ${e.type || e.tp}</div>`
    ).join("");
}
...
function showDicePopup(res, tg, auto) {
    ...
    po.innerHTML = `
        ...
        <div class="lc-popup-details">
            <div>Цель: <strong>${tg}</strong></div>
            <div>Тип: ${res.type} · Эякуляция: ${res.ejac}</div>
            <div>Шанс: <strong>${res.chance}%</strong></div>
        </div>
        <div class="lc-popup-result ${cls}">${res.roll} / ${res.chance}</div>
        <div class="lc-popup-verdict ${cls}">${res.result ? "🤰 ЗАЧАТИЕ!" : "Не в этот раз"}</div>
        <div class="lc-popup-actions">
            <button class="lc-btn lc-btn-success" id="lc-dp-ok">✓ OK</button>
            <button class="lc-btn" id="lc-dp-re">🎲 Перебросить</button>
            <button class="lc-btn lc-btn-danger" id="lc-dp-no">✕ Отмена</button>
        </div>`;

    document.body.appendChild(ov);
    document.body.appendChild(po);

    document.getElementById("lc-dp-ok").addEventListener("click", () => {
        if (res.result) {
            const p = S().characters[tg];
            if (p && canGetPregnant(p)) {
                new PregManager(p).start(res.parts?.find(x => x !== tg) || "?", 1);
                saveSettingsDebounced();
                rebuild();
            }
        }
    ...
    document.getElementById("lc-dp-re").addEventListener("click", () => {
        ov.remove(); po.remove();
        const nr = Intimacy.roll(tg, { parts: res.parts, tp: res.type, ej: res.ejac, co: false, nc: false, auto });
        showDicePopup(nr, tg, auto);
    });
...
function showBabyForm(parent, father, existing, idx, standalone) {
    ...
    fm.innerHTML = `
        <h3 class="lc-popup-title">${isEdit ? "✏️ Редактирование" : "👶 Новый ребёнок"}</h3>
        <div class="lc-editor-grid">
            <div class="lc-editor-field"><label>Имя</label><input type="text" id="lc-bf-name" class="lc-input" value="${b.name || ''}"></div>
            <div class="lc-editor-field"><label>Пол</label><select id="lc-bf-sex" class="lc-select"><option value="random">Случайно</option><option value="M" ${b.sex==='M'?'selected':''}>M ♂</option><option value="F" ${b.sex==='F'?'selected':''}>F ♀</option></select></div>
            <div class="lc-editor-field"><label>Втор. пол</label><select id="lc-bf-sec" class="lc-select"><option value="">Нет</option><option value="random">Случайно</option><option value="alpha">Alpha</option><option value="beta">Beta</option><option value="omega">Omega</option></select></div>
            <div class="lc-editor-field"><label>Глаза</label><input type="text" id="lc-bf-eyes" class="lc-input" value="${b.eyeColor || ''}"></div>
            <div class="lc-editor-field"><label>Волосы</label><input type="text" id="lc-bf-hair" class="lc-input" value="${b.hairColor || ''}"></div>
            ${isEdit ? '<div class="lc-editor-field"><label>Возраст (дни)</label><input type="number" id="lc-bf-age" class="lc-input" value="' + (b.ageDays || 0) + '"></div>' : ''}
            ${standalone ? '<div class="lc-editor-field"><label>Мать</label><select id="lc-bf-mo" class="lc-select"><option value="?">?</option>' + charOpts() + '</select></div><div class="lc-editor-field"><label>Отец</label><select id="lc-bf-fa" class="lc-select"><option value="?">?</option>' + charOpts() + '</select></div><div class="lc-editor-field"><label>Записать к</label><select id="lc-bf-to" class="lc-select">' + charOpts() + '</select></div>' : ''}
        </div>
        <div class="lc-popup-actions">
            <button class="lc-btn lc-btn-success" id="lc-bf-save">${isEdit ? '💾 Сохранить' : '👶 Создать'}</button>
            <button class="lc-btn" id="lc-bf-cancel">Отмена</button>
        </div>`;
    ...
    document.getElementById("lc-bf-save").addEventListener("click", () => {
        ...
        let sex = document.getElementById("lc-bf-sex").value;
        if (sex === "random") sex = Math.random() < 0.5 ? "M" : "F";
        let sec = document.getElementById("lc-bf-sec").value;
        if (sec === "random") { const r = Math.random(); sec = r < 0.25 ? "alpha" : r < 0.75 ? "beta" : "omega"; }
        const eyes = document.getElementById("lc-bf-eyes").value.trim();
        const hair = document.getElementById("lc-bf-hair").value.trim();

        if (isEdit) {
            const baby = s.characters[parent]?.babies?.[idx];
            if (baby) {
                baby.name = name; baby.sex = sex; baby.secondarySex = sec || null;
                if (eyes) baby.eyeColor = eyes; if (hair) baby.hairColor = hair;
                const ageEl = document.getElementById("lc-bf-age");
                if (ageEl) { baby.ageDays = parseInt(ageEl.value) || 0; new BabyManager(baby).update(); }
                saveSettingsDebounced();
                rebuild();
            }
        } else if (standalone) {
            const mo = document.getElementById("lc-bf-mo")?.value || "?";
            const fa = document.getElementById("lc-bf-fa")?.value || "?";
            const to = document.getElementById("lc-bf-to")?.value;
            if (to && s.characters[to]) {
                const baby = BabyManager.gen(s.characters[mo], fa, { name, sex, secondarySex: sec || null, eyeColor: eyes, hairColor: hair });
                baby.mother = mo; baby.father = fa;
                s.characters[to].babies.push(baby);
                Rels.addBirth(mo, fa, name);
                saveSettingsDebounced();
                rebuild();
                toastr.success("Ребёнок создан: " + name);
            }
        } else {
            // Birth delivery
            const mo = s.characters[parent];
            if (mo) {
                const baby = BabyManager.gen(mo, father, { name, sex, secondarySex: sec || null, eyeColor: eyes, hairColor: hair });
                mo.babies.push(baby);
                Rels.addBirth(parent, father, name);
                const lm = new LaborManager(mo);
                lm.deliver();
                if (lm.l.babiesDelivered >= lm.l.totalBabies) lm.end();
                saveSettingsDebounced();
                rebuild();
                toastr.success("Родился: " + name + " (" + (sex === "M" ? "♂" : "♀") + ")!");
            }
        }
        ov.remove(); fm.remove();
    });
...
function bindAll() {
    ...
    document.getElementById("lc-reparse")?.addEventListener("click", () => { syncChars(); rebuild(); toastr.success("Чат перечитан!"); });
    document.getElementById("lc-char-list")?.addEventListener("click", function (e) {
        ...
        if (db && confirm("Удалить?")) { delete s.characters[db.dataset.char]; saveSettingsDebounced(); rebuild(); }
    });
    ...
    // Rels
    document.getElementById("lc-rel-add")?.addEventListener("click", () => {
        const c1 = document.getElementById("lc-rel-c1")?.value, c2 = document.getElementById("lc-rel-c2")?.value, tp = document.getElementById("lc-rel-tp")?.value;
        if (!c1 || !c2 || c1 === c2) return;
        Rels.add(c1, c2, tp, document.getElementById("lc-rel-n")?.value);
        document.getElementById("lc-rel-n").value = "";
        renderRels();
    });

    // Cycle
    document.getElementById("lc-cyc-char")?.addEventListener("change", renderCycle);
    document.getElementById("lc-cyc-setday")?.addEventListener("click", () => {
        const p = s.characters[document.getElementById("lc-cyc-char")?.value];
        if (!p?.cycle?.enabled) return;
        const d = parseInt(document.getElementById("lc-cyc-day")?.value);
        if (d >= 1 && d <= p.cycle.length) { new CycleManager(p).setDay(d); saveSettingsDebounced(); renderCycle(); renderDash(); }
    });
    ["mens", "foll", "ovul", "lut"].forEach(ph => {
        document.getElementById("lc-cyc-" + ph)?.addEventListener("click", () => {
            const p = s.characters[document.getElementById("lc-cyc-char")?.value];
            if (!p?.cycle?.enabled) return;
            new CycleManager(p).setPhase({ mens: "menstruation", foll: "follicular", ovul: "ovulation", lut: "luteal" }[ph]);
            saveSettingsDebounced(); renderCycle(); renderDash();
        });
    });
    document.getElementById("lc-cyc-skip")?.addEventListener("click", () => {
        const p = s.characters[document.getElementById("lc-cyc-char")?.value];
        if (!p?.cycle?.enabled) return;
        p.cycle.currentDay = 1; p.cycle.cycleCount++; saveSettingsDebounced(); renderCycle(); renderDash();
    });
    ...
    // Intimacy
    document.getElementById("lc-int-log")?.addEventListener("click", () => {
        const t = document.getElementById("lc-int-t")?.value; if (!t) return;
        Intimacy.log({ parts: [t, document.getElementById("lc-int-p")?.value].filter(Boolean), type: document.getElementById("lc-int-tp")?.value, ejac: document.getElementById("lc-int-ej")?.value });
        renderIntim();
    });
    document.getElementById("lc-int-roll")?.addEventListener("click", () => {
        const t = document.getElementById("lc-int-t")?.value; if (!t) return;
        const r = Intimacy.roll(t, { parts: [t, document.getElementById("lc-int-p")?.value].filter(Boolean), tp: document.getElementById("lc-int-tp")?.value, ej: document.getElementById("lc-int-ej")?.value });
        if (r.reason === "not_eligible") { toastr.warning("Этот персонаж не может забеременеть!"); return; }
        showDicePopup(r, t, false); renderDice();
    });
    ...
    document.getElementById("lc-preg-labor")?.addEventListener("click", () => { const p = s.characters[document.getElementById("lc-preg-char")?.value]; if (!p?.pregnancy?.active) return; new LaborManager(p).start(); saveSettingsDebounced(); renderLabor(); renderDash(); toastr.warning("Роды начались!"); });
    ...
    document.getElementById("lc-baby-list")?.addEventListener("click", function (e) {
        const eb = e.target.closest(".lc-baby-edit"), db = e.target.closest(".lc-baby-del");
        if (eb) { const baby = s.characters[eb.dataset.p]?.babies?.[parseInt(eb.dataset.i)]; if (baby) showBabyForm(eb.dataset.p, baby.father, baby, parseInt(eb.dataset.i)); }
        if (db && confirm("Удалить?")) { s.characters[db.dataset.p]?.babies?.splice(parseInt(db.dataset.i), 1); saveSettingsDebounced(); renderBabies(); }
    });
    ...
    document.getElementById("lc-prof-list")?.addEventListener("click", function (e) {
        const lb = e.target.closest(".lc-prof-load"), db = e.target.closest(".lc-prof-del");
        if (lb) { const pr = s.chatProfiles[lb.dataset.id]; if (pr) { s.characters = JSON.parse(JSON.stringify(pr.characters || {})); s.relationships = JSON.parse(JSON.stringify(pr.relationships || [])); s.worldDate = { ...(pr.worldDate || DEFAULTS.worldDate) }; s.currentChatId = lb.dataset.id; saveSettingsDebounced(); rebuild(); toastr.success("Загружено!"); } }
        if (db && confirm("Удалить?")) { Profiles.del(db.dataset.id); renderProfs(); }
    });
    ...
    // Export/Import/Reset
    document.getElementById("lc-exp")?.addEventListener("click", () => {
        const b = new Blob([JSON.stringify(s, null, 2)], { type: "application/json" });
        const u = URL.createObjectURL(b); const a = document.createElement("a"); a.href = u; a.download = "lifecycle_" + Date.now() + ".json";
        document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(u);
    });
    document.getElementById("lc-imp")?.addEventListener("click", () => {
        const inp = document.createElement("input"); inp.type = "file"; inp.accept = ".json";
        inp.addEventListener("change", e => { const f = e.target.files[0]; if (!f) return; const r = new FileReader(); r.onload = ev => { try { extension_settings[EXT] = deep(DEFAULTS, JSON.parse(ev.target.result)); saveSettingsDebounced(); document.getElementById("lifecycle-panel")?.remove(); init(); } catch (er) { toastr.error("JSON: " + er.message); } }; r.readAsText(f); });
        inp.click();
    });
    document.getElementById("lc-rst")?.addEventListener("click", () => { if (!confirm("Полный сброс?")) return; extension_settings[EXT] = JSON.parse(JSON.stringify(DEFAULTS)); saveSettingsDebounced(); document.getElementById("lifecycle-panel")?.remove(); init(); });
}
...
function openEditor(name) {
    const s = S(), p = s.characters[name]; if (!p) return;
    editName = name;
...
function saveEditor() {
    if (!editName) return;
    const s = S(), p = s.characters[editName]; if (!p) return;
...
function genWidget() {
    ...
    let h = '<div class="lc-status-widget"><div class="lc-sw-header"><span>🌿 LifeCycle</span><span class="lc-sw-arrow">▼</span></div><div class="lc-sw-body">';
    h += `<div class="lc-sw-date">📅 ${fmt(s.worldDate)}</div>`;
    ...
    for (const [n, p] of chars) {
        ...
        if (s.modules.labor && p.labor?.active) {
            const lm = new LaborManager(p), prog = Math.round((p.labor.dilation / 10) * 100);
            h += `<div class="lc-sw-block lc-sw-labor-block"><div class="lc-sw-block-title">🏥 ${LABOR_LABELS[p.labor.stage]}</div><div class="lc-sw-row">Раскрытие: ${p.labor.dilation}/10 см</div><div class="lc-sw-mini-progress"><div class="lc-sw-mini-fill labor" style="width:${prog}%"></div></div><div class="lc-sw-desc">${lm.desc()}</div></div>`;
        }
        // Pregnancy
            ...
            h += `<div class="lc-sw-block lc-sw-preg-block"><div class="lc-sw-block-title">🤰 Нед. ${p.pregnancy.week}/${p.pregnancy.maxWeeks} (T${pm.tri()})</div><div class="lc-sw-mini-progress"><div class="lc-sw-mini-fill preg" style="width:${prog}%"></div></div><div class="lc-sw-row">~${pm.size()} · Шевеления: ${pm.moves()}</div><div class="lc-sw-symptoms">${pm.symptoms().join(", ")}</div></div>`;
        }

        // Heat
        if (s.modules.auOverlay && s.auPreset === "omegaverse" && p.heat?.active) {
            const hr = new HeatRutManager(p);
            h += `<div class="lc-sw-block lc-sw-heat-block"><div class="lc-sw-block-title">🔥 ${HeatRutManager.HP[hr.hPhase()]} · День ${p.heat.currentDay}/${p.heat.duration}</div><div class="lc-sw-symptoms">${hr.hSym().join(", ")}</div></div>`;
        }
        // Rut
        if (s.modules.auOverlay && s.auPreset === "omegaverse" && p.rut?.active) {
            const hr = new HeatRutManager(p);
            h += `<div class="lc-sw-block lc-sw-rut-block"><div class="lc-sw-block-title">💢 ${HeatRutManager.RP[hr.rPhase()]} · День ${p.rut.currentDay}/${p.rut.duration}</div><div class="lc-sw-symptoms">${hr.rSym().join(", ")}</div></div>`;
        }

        // Oviposition
        if (s.auSettings.oviposition?.enabled && p.oviposition?.active) {
            const om = new OviManager(p), prog = om.progress();
            h += `<div class="lc-sw-block lc-sw-ovi-block"><div class="lc-sw-block-title">🥚 ${OviManager.PHASES[p.oviposition.phase]}</div><div class="lc-sw-mini-progress"><div class="lc-sw-mini-fill ovi" style="width:${prog}%"></div></div><div class="lc-sw-row">Яиц: ${p.oviposition.eggCount} (${p.oviposition.fertilizedCount} ферт.)</div><div class="lc-sw-symptoms">${om.symptoms().join(", ")}</div></div>`;
        }
        ...
        if (s.modules.cycle && p.cycle?.enabled && !p.pregnancy?.active && !p.labor?.active) {
            ...
            h += `<div class="lc-sw-block lc-sw-cycle-block"><div class="lc-sw-row">${cm.emoji(cm.phase())} ${cm.label(cm.phase())} · День ${p.cycle.currentDay}/${p.cycle.length} · <span class="lc-sw-fert ${fc}">${Math.round(f * 100)}%</span></div></div>`;
        }

        // Babies
        if (s.modules.baby && p.babies?.length > 0) {
            h += '<div class="lc-sw-block lc-sw-baby-block">';
            p.babies.forEach(b => { const bm = new BabyManager(b); h += `<div class="lc-sw-baby-row">${b.sex === "M" ? "♂" : "♀"} <strong>${b.name || "?"}</strong> · ${bm.age()}</div>`; });
            h += '</div>';
        }
...
function onMessage(msgIdx) {
    ...
    if (s.autoTimeProgress && !s.worldDate.frozen) {
        ...
        if (tp) {
            if (s.timeParserConfirmation) {
                if (confirm("LifeCycle: обнаружен сдвиг времени. Применить?")) { TimeParse.apply(tp); rebuild(); }
            } else {
                TimeParse.apply(tp);
                rebuild();
            }
    ...
    if (s.autoDetectIntimacy && s.modules.intimacy) {
        const det = IntimacyDetector.detect(msg.mes, s.characters);
        if (det?.detected) {
...
async function init() {
    try {
        if (!extension_settings[EXT]) {
            ...
            extension_settings[EXT] = deep(JSON.parse(JSON.stringify(DEFAULTS)), extension_settings[EXT]);
        }
        ...
        if (target) {
            target.insertAdjacentHTML("beforeend", genHTML());
        } else {
            console.warn("[LifeCycle] Could not find extensions_settings container!");
            return;
        }
        ...
        console.log("[LifeCycle v0.8.1] Loaded successfully!");
    } catch (err) {
