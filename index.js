// BunnyCycle v1.1.0 — Full index.js (AU Tuning Update)
...
const DEFAULTS = {
    ...
    auSettings: {
        ...
        fantasy: {
            pregnancyByRace: {
                human: 40, elf: 60, dwarf: 35, orc: 32,
                demon: 28, vampire: 50, werewolf: 9,
                fairy: 20, dragon: 80, halfling: 38
            },
...
var LLM = {
    ...
    parseJSON: function (text) {
        ...
        try { return JSON.parse(match[0]); } catch (e) { console.warn("[BunnyCycle] JSON parse error:", e.message); return null; }
    }
};
...
var CharAnalyzer = {
    ...
    analyze: async function (name, charObj, isUser) {
        ...
        if (parsed) { this._cache[cacheKey] = parsed; console.log("[BunnyCycle] Character analysis for " + name + ":", parsed); }
        return parsed;
    },
...
var ChatAnalyzer = {
    ...
    analyze: async function (messages, characterNames) {
        ...
        var raw = await LLM.call(this.SYSTEM, userPrompt);
        var parsed = LLM.parseJSON(raw);
        if (parsed) { this._cache[cacheKey] = parsed; this._lastMessageCount = messages.length; console.log("[BunnyCycle] Chat analysis:", parsed); }
        return parsed;
    },
...
CycleManager.prototype.phase = function () {
    ...
    var d = this.c.currentDay, len = this.c.length, mDur = this.c.menstruationDuration;
...
CycleManager.prototype.fertility = function () {
    ...
    var bonus = 0;
    var s = S();
    if (s.modules.auOverlay && s.auPreset === "omegaverse" && this.p.heat && this.p.heat.active) bonus = s.auSettings.omegaverse.heatFertilityBonus;
...
CycleManager.prototype.discharge = function () {
    return ({ menstruation: "менструальные выделения", follicular: "скудные, сухо", ovulation: "обильные, прозрачные, тягучие (как яичный белок)", luteal: "густые, кремообразные" })[this.phase()] || "обычные";
};
...
// ========================
// HEAT/RUT MANAGER (UPDATED with AU settings)
// ========================
...
HeatRutManager.prototype.heatPhase = function () {
    ...
    var preDays = cfg.preHeatDays || 1;
    var postStart = (cfg.heatDuration || 5) - (cfg.postHeatDays || 1);
    if (h.active) {
        if (h.currentDay <= preDays) return "preHeat";
        if (h.currentDay <= postStart) return "heat";
        return "postHeat";
    }
...
HeatRutManager.prototype.rutPhase = function () {
    ...
    if (!r) return "rest";
    var cfg = S().auSettings.omegaverse || {};
    var preDays = cfg.preRutDays || 1;
    var postStart = (cfg.rutDuration || 4) - (cfg.postRutDays || 1);
    if (r.active) {
        if (r.currentDay <= preDays) return "preRut";
        if (r.currentDay <= postStart) return "rut";
        return "postRut";
    }
...
HeatRutManager.prototype.advanceHeat = function (days) {
    ...
    for (var i = 0; i < days; i++) {
        if (h.active) {
            ...
            if (h.daysSinceLast >= h.cycleDays) { h.active = true; h.currentDay = 1; }
        }
    }
};
...
function BondManager(profile) {
    this.p = profile;
    if (!profile.bond) {
        profile.bond = { bonded: false, partner: null, type: null, strength: 0, daysSinceSeparation: 0, withdrawalActive: false, markLocation: "" };
    }
    this.b = profile.bond;
}
...
BondManager.prototype.createBond = function (partnerName) {
    ...
    this.b.type = cfg.bondingType || "bite";
    this.b.strength = 50;
...
BondManager.prototype.breakBond = function () {
    var cfg = S().auSettings.omegaverse;
    if (!cfg.bondBreakable && this.b.bonded) {
        toastr.warning("Связь нельзя разорвать (настройка AU)!");
        return false;
    }
    var partner = this.b.partner;
    this.b.bonded = false; this.b.partner = null; this.b.strength = 0;
    this.b.withdrawalActive = true; this.b.daysSinceSeparation = 0;
    if (partner) {
        var s = S();
        var pp = s.characters[partner];
...
BondManager.prototype.advance = function (days) {
    ...
    var cfg = S().auSettings.omegaverse;
    if (this.b.bonded) {
        this.b.strength = Math.min(100, this.b.strength + days * 2);
    }
    if (this.b.withdrawalActive) {
        this.b.daysSinceSeparation += days;
        if (this.b.daysSinceSeparation >= (cfg.bondWithdrawalDays || 7)) {
            this.b.withdrawalActive = false;
        }
    }
};
...
PregManager.prototype.start = function (father, count) {
    ...
    if (s.modules.auOverlay && s.auPreset === "omegaverse" && Math.random() < (s.auSettings.omegaverse.twinChance || 0)) baseCount = 2;
...
PregManager.prototype.size = function () {
    var map = [[4, "маковое зерно"], [8, "малина"], [12, "лайм"], [16, "авокадо"], [20, "банан"], [28, "баклажан"], [36, "дыня"], [40, "арбуз"]];
    var result = "эмбрион";
    for (var i = 0; i < map.length; i++) { if (this.pr.week >= map[i][0]) result = map[i][1]; }
    return result;
};
PregManager.prototype.symptoms = function () {
    var w = this.pr.week, result = [];
...
BabyManager.generate = function (mother, fatherName, overrides) {
    ...
    return { name: ov.name || "", sex: sex, secondarySex: null, birthWeight: bw, currentWeight: bw, ageDays: ov.ageDays || 0,
        eyeColor: ov.eyeColor || (mother ? mother.eyeColor : "") || "", hairColor: ov.hairColor || (mother ? mother.hairColor : "") || "",
        mother: (mother ? mother.name : ov.mother) || "?", father: fatherName || ov.father || "?", state: "новорождённый",
...
BabyManager.prototype.milestones = function () {
    var d = this.b.ageDays, result = [];
    if (d >= 42) result.push("социальная улыбка"); if (d >= 90) result.push("держит голову");
    if (d >= 150) result.push("переворачивается"); if (d >= 180) result.push("сидит");
    if (d >= 270) result.push("ползает"); if (d >= 365) result.push("первые шаги");
    if (d >= 450) result.push("первые слова"); if (d >= 730) result.push("бегает, говорит фразы");
    return result;
};
...
var Intimacy = {
    ...
    roll: function (targetName, data) {
        ...
        if (data.noCondom) { /* no reduction */ } else if (data.condom) { fertility *= 0.15; } else { fertility *= (1 - ce); }
        ...
        var diceRoll = roll100(); var success = diceRoll <= chance;
...
var Rels = {
    ...
    add: function (c1, c2, type, notes) {
        ...
        var exists = s.relationships.some(function (r) { return r.char1 === c1 && r.char2 === c2 && r.type === type; });
        if (exists) return;
        s.relationships.push({ id: makeId(), char1: c1, char2: c2, type: type, notes: notes || "" }); saveSettingsDebounced();
    },
...
var Profiles = {
    ...
    save: function () {
        var s = S(); var cid = this.id(); if (!cid) return; s.currentChatId = cid; if (!s.chatProfiles) s.chatProfiles = {};
        s.chatProfiles[cid] = { characters: JSON.parse(JSON.stringify(s.characters)), relationships: JSON.parse(JSON.stringify(s.relationships || [])),
            worldDate: JSON.parse(JSON.stringify(s.worldDate)), diceLog: JSON.parse(JSON.stringify(s.diceLog || [])), intimacyLog: JSON.parse(JSON.stringify(s.intimacyLog || [])) };
        saveSettingsDebounced();
    },
    load: function () {
        ...
        if (s.currentChatId && Object.keys(s.characters).length > 0) {
            ...
            s.chatProfiles[s.currentChatId] = { characters: JSON.parse(JSON.stringify(s.characters)), relationships: JSON.parse(JSON.stringify(s.relationships || [])),
                worldDate: JSON.parse(JSON.stringify(s.worldDate)), diceLog: JSON.parse(JSON.stringify(s.diceLog || [])), intimacyLog: JSON.parse(JSON.stringify(s.intimacyLog || [])) };
        }
        s.currentChatId = cid;
        if (s.chatProfiles && s.chatProfiles[cid]) {
            var pr = s.chatProfiles[cid]; s.characters = JSON.parse(JSON.stringify(pr.characters || {})); s.relationships = JSON.parse(JSON.stringify(pr.relationships || []));
            s.worldDate = JSON.parse(JSON.stringify(pr.worldDate || DEFAULTS.worldDate)); s.diceLog = JSON.parse(JSON.stringify(pr.diceLog || [])); s.intimacyLog = JSON.parse(JSON.stringify(pr.intimacyLog || []));
        } else { s.characters = {}; s.relationships = []; s.diceLog = []; s.intimacyLog = []; }
        saveSettingsDebounced(); return true;
    },
    list: function () {
        var s = S(); var profiles = s.chatProfiles || {};
        return Object.keys(profiles).map(function (id) { var p = profiles[id]; return { id: id, count: Object.keys(p.characters || {}).length, date: p.worldDate ? formatDate(p.worldDate) : "-", isCurrent: id === s.currentChatId }; });
    },
    ...
    generate: function () {
        ...
        for (var i = 0; i < charNames.length; i++) {
            ...
            if (s.modules.cycle && p.cycle && p.cycle.enabled && !(p.pregnancy && p.pregnancy.active)) {
                var cm = new CycleManager(p); lines.push("Cycle D" + p.cycle.currentDay + "/" + p.cycle.length + " " + cm.label(cm.phase()) + " Fert:" + Math.round(cm.fertility() * 100) + "%");
            }
            if (s.modules.pregnancy && p.pregnancy && p.pregnancy.active) { var pm = new PregManager(p); lines.push("PREGNANT W" + p.pregnancy.week + "/" + p.pregnancy.maxWeeks + " " + pm.size()); }
            if (s.modules.labor && p.labor && p.labor.active) lines.push("LABOR: " + LABOR_LABELS[p.labor.stage]);
            ...
            if (s.modules.baby && p.babies && p.babies.length > 0) {
                for (var j = 0; j < p.babies.length; j++) { var b = p.babies[j]; lines.push("Child: " + (b.name || "?") + " " + new BabyManager(b).age()); }
            }
        }
        // AU context
        if (s.modules.auOverlay) {
...
var TimeParse = {
    parse: function (msg) {
        ...
        var patterns = [
            ...
            [/прошл[оа]\s+(\d+)\s+(?:недел|нед)/gi, 7], [/через\s+(\d+)\s+(?:недел|нед)/gi, 7], [/прошл[оа]\s+(\d+)\s+(?:месяц|мес)/gi, 30],
    ...
    advanceAll: function (days) {
        ...
        for (var i = 0; i < charNames.length; i++) {
            ...
            if (s.modules.baby && p.babies && p.babies.length > 0) { for (var j = 0; j < p.babies.length; j++) { p.babies[j].ageDays += days; new BabyManager(p.babies[j]).update(); } }
        }
    }
};

// ========================
// CHARACTER PROFILE FACTORY (UPDATED with bond)
// ========================
...
function getActiveChars() {
    ...
    if (ctx.characterId !== undefined && ctx.characters) { var ch = ctx.characters[ctx.characterId]; if (ch) result.push({ name: ch.name, obj: ch, isUser: false }); }
    if (ctx.groups && ctx.groupId) {
        var group = ctx.groups.find(function (g) { return g.id === ctx.groupId; });
        if (group && group.members) {
            for (var i = 0; i < group.members.length; i++) { var avatar = group.members[i]; var found = ctx.characters.find(function (c) { return c.avatar === avatar; }); if (found && !result.some(function (r) { return r.name === found.name; })) result.push({ name: found.name, obj: found, isUser: false }); }
        }
    }
...
async function syncChars() {
    ...
    try {
        ...
        for (var i = 0; i < active.length; i++) { var c = active[i]; if (!s.characters[c.name]) { s.characters[c.name] = makeProfile(c.name, c.isUser, "F"); changed = true; } }
        // Ensure bond field exists for old profiles
        var allNames = Object.keys(s.characters);
        for (var bn = 0; bn < allNames.length; bn++) { if (!s.characters[allNames[bn]].bond) s.characters[allNames[bn]].bond = { bonded: false, partner: null, type: null, strength: 0, daysSinceSeparation: 0, withdrawalActive: false, markLocation: "" }; }

        if (s.autoParseCharInfo && s.useLLMParsing) {
            for (var j = 0; j < active.length; j++) {
                var ch = active[j]; var pr = s.characters[ch.name]; if (pr._mB && pr._mE && pr._mH) continue;
            ...
            if (ChatAnalyzer.shouldReanalyze(msgs) || Object.keys(ChatAnalyzer._cache).length === 0) {
                ...
                if (chatResult && chatResult.children) {
                    for (var ci = 0; ci < chatResult.children.length; ci++) {
                        ...
                        var motherProfile = child.mother ? s.characters[child.mother] : null; var fatherProfile = child.father ? s.characters[child.father] : null;
                        ...
                        if (!alreadyExists) {
                            attachTo.babies.push({ name: child.name, sex: child.sex || "F", secondarySex: null, birthWeight: 3200, currentWeight: 5000, ageDays: 30, eyeColor: "", hairColor: "", mother: child.mother || "?", father: child.father || "?", state: "младенец", birthDate: JSON.parse(JSON.stringify(s.worldDate)) });
...
// HTML GENERATION
// ========================
function charOptions() { var names = Object.keys(S().characters); var html = ""; for (var i = 0; i < names.length; i++) html += '<option value="' + names[i] + '">' + names[i] + '</option>'; return html; }
function relTypeOptions() { var html = ""; for (var i = 0; i < REL_TYPES.length; i++) html += '<option value="' + REL_TYPES[i] + '">' + REL_TYPES[i] + '</option>'; return html; }

function generateHTML() {
    var s = S(); var co = charOptions(); var rto = relTypeOptions(); var h = [];
    h.push('<div id="bunnycycle-panel" class="lifecycle-panel' + (s.panelCollapsed ? ' collapsed' : '') + '">');
    // Header
    h.push('<div id="bunnycycle-header-toggle" class="lifecycle-header"><div class="lifecycle-header-title"><span class="lc-collapse-arrow">' + (s.panelCollapsed ? '▶' : '▼') + '</span><h3>🐰 BunnyCycle</h3><span class="lc-version">v1.1</span></div><div class="lifecycle-header-actions"><label class="lc-switch"><input type="checkbox" id="lc-enabled"' + (s.enabled ? ' checked' : '') + '><span class="lc-switch-slider"></span></label></div></div>');
    ...
    var tabs = [["chars","👥","Перс"],["rels","💞","Семья"],["cycle","🔴","Цикл"],["hr","🔥","Течка"],["intim","💕","Интим"],["preg","🤰","Берем"],["labor","🏥","Роды"],["baby","👶","Дети"],["ovi","🥚","Яйца"],["au","🌐","AU"],["profs","💾","Проф"],["sett","⚙️","Настр"]];
    for (var t = 0; t < tabs.length; t++) h.push('<button class="lifecycle-tab' + (t === 0 ? ' active' : '') + '" data-tab="' + tabs[t][0] + '"><span class="tab-icon">' + tabs[t][1] + '</span>' + tabs[t][2] + '</button>');
    h.push('</div>');
    ...
    var edFields = [['Пол','<select class="lc-select" id="lc-ed-bio"><option value="F">♀</option><option value="M">♂</option></select>'],['2-й пол','<select class="lc-select" id="lc-ed-sec"><option value="">-</option><option value="alpha">α</option><option value="beta">β</option><option value="omega">Ω</option></select>'],['Раса','<select class="lc-select" id="lc-ed-race"><option value="human">Человек</option><option value="elf">Эльф</option><option value="orc">Орк</option><option value="demon">Демон</option><option value="vampire">Вампир</option><option value="werewolf">Оборотень</option><option value="fairy">Фея</option><option value="dragon">Дракон</option><option value="dwarf">Гном</option><option value="halfling">Полурослик</option></select>'],['Контрацепция','<select class="lc-select" id="lc-ed-contra"><option value="none">Нет</option><option value="condom">Презерв.</option><option value="pill">Таблетки</option><option value="iud">ВМС</option><option value="withdrawal">ППА</option></select>'],['Глаза','<input class="lc-input" id="lc-ed-eyes">'],['Волосы','<input class="lc-input" id="lc-ed-hair">'],['Сложность','<select class="lc-select" id="lc-ed-diff"><option value="easy">Лёгкие</option><option value="normal">Обычные</option><option value="hard">Тяжёлые</option></select>'],['Включён','<input type="checkbox" id="lc-ed-on">'],['Цикл','<input type="checkbox" id="lc-ed-cyc">'],['Длина цикла','<input type="number" class="lc-input" id="lc-ed-clen" min="21" max="45">'],['Менструация','<input type="number" class="lc-input" id="lc-ed-mdur" min="2" max="10">'],['Нерегулярность','<input type="number" class="lc-input" id="lc-ed-irreg" min="0" max="7">']];
    for (var ef = 0; ef < edFields.length; ef++) h.push('<div class="lc-editor-field"><label>' + edFields[ef][0] + '</label>' + edFields[ef][1] + '</div>');
    h.push('</div><div class="lc-editor-actions"><button class="lc-btn lc-btn-success" id="lc-ed-save">💾 Сохранить</button><button class="lc-btn" id="lc-ed-cancel">Отмена</button></div></div></div>');

    // TAB: Relationships
    h.push('<div class="lifecycle-tab-content" data-tab="rels"><div class="lc-row" style="margin-bottom:8px;flex-wrap:wrap"><select class="lc-select lc-char-select" id="lc-rel-c1">' + co + '</select><select class="lc-select" id="lc-rel-tp">' + rto + '</select><select class="lc-select lc-char-select" id="lc-rel-c2">' + co + '</select><input class="lc-input" id="lc-rel-n" placeholder="Заметка" style="max-width:80px"><button class="lc-btn lc-btn-sm" id="lc-rel-add">➕</button></div><div id="lc-rel-list"></div></div>');
    ...
    // TAB: Intimacy
    h.push('<div class="lifecycle-tab-content" data-tab="intim"><div class="lc-section"><div class="lc-row"><label>Цель</label><select class="lc-select lc-char-select" id="lc-int-t">' + co + '</select></div><div class="lc-row"><label>Партнёр</label><select class="lc-select lc-char-select" id="lc-int-p">' + co + '</select></div><div class="lc-row"><label>Тип</label><select class="lc-select" id="lc-int-tp"><option value="vaginal">Вагин.</option><option value="anal">Анал.</option><option value="oral">Орал.</option></select></div><div class="lc-row"><label>Эякуляция</label><select class="lc-select" id="lc-int-ej"><option value="inside">Внутрь</option><option value="outside">Наружу</option><option value="unknown">?</option></select></div><div class="lc-btn-group"><button class="lc-btn" id="lc-int-log">📝 Записать</button><button class="lc-btn lc-btn-primary" id="lc-int-roll">🎲 Бросок</button></div></div><div id="lc-dice-log" class="lc-scroll"></div><div id="lc-intim-log" class="lc-scroll"></div></div>');

    // TAB: Pregnancy
    h.push('<div class="lifecycle-tab-content" data-tab="preg"><select class="lc-select lc-char-select" id="lc-preg-char" style="margin-bottom:6px">' + co + '</select><div id="lc-preg-panel"></div><div class="lc-btn-group" style="margin-top:6px"><button class="lc-btn lc-btn-sm" id="lc-preg-adv">+1 нед</button><button class="lc-btn lc-btn-sm" id="lc-preg-set">Уст. нед.</button><button class="lc-btn lc-btn-sm lc-btn-danger" id="lc-preg-labor">→ Роды</button><button class="lc-btn lc-btn-sm lc-btn-danger" id="lc-preg-end">Прервать</button></div></div>');

    // TAB: Labor
    h.push('<div class="lifecycle-tab-content" data-tab="labor"><select class="lc-select lc-char-select" id="lc-labor-char" style="margin-bottom:6px">' + co + '</select><div id="lc-labor-panel"></div><div class="lc-btn-group" style="margin-top:6px"><button class="lc-btn lc-btn-sm" id="lc-labor-adv">→ Стадия</button><button class="lc-btn lc-btn-sm lc-btn-success" id="lc-labor-deliver">👶 Родить</button><button class="lc-btn lc-btn-sm lc-btn-danger" id="lc-labor-end">Завершить</button></div></div>');

    // TAB: Baby
    h.push('<div class="lifecycle-tab-content" data-tab="baby"><div class="lc-row" style="margin-bottom:6px"><select class="lc-select lc-char-select" id="lc-baby-par">' + co + '</select><button class="lc-btn lc-btn-sm" id="lc-baby-create">➕ Создать</button></div><div id="lc-baby-list"></div></div>');
    ...
    // === TAB: AU SETTINGS (NEW) ===
    h.push('<div class="lifecycle-tab-content" data-tab="au">');
    h.push('<div id="lc-au-panel"></div>');
    h.push('</div>');
    ...
    var modChecks = [["lc-mc",s.modules.cycle,"🔴 Цикл"],["lc-mp",s.modules.pregnancy,"🤰 Беременность"],["lc-ml",s.modules.labor,"🏥 Роды"],["lc-mb",s.modules.baby,"👶 Дети"],["lc-mi",s.modules.intimacy,"💕 Интимность"],["lc-mau",s.modules.auOverlay,"🌐 AU Оверлей"],["lc-ovi-on",s.auSettings.oviposition.enabled,"🥚 Oviposition"]];
    for (var mc = 0; mc < modChecks.length; mc++) h.push('<label class="lc-checkbox"><input type="checkbox" id="' + modChecks[mc][0] + '"' + (modChecks[mc][1] ? ' checked' : '') + '><span>' + modChecks[mc][2] + '</span></label>');
    h.push('</div>');
    h.push('<div class="lc-section"><h4>Автоматизация</h4>');
    var autoChecks = [["lc-sa",s.autoSyncCharacters,"Авто-синхр."],["lc-sp",s.autoParseCharInfo,"Парсинг карточек"],["lc-sllm",s.useLLMParsing,"🧠 AI-анализ (LLM)"],["lc-sc",s.parseFullChat,"Парсинг чата"],["lc-sd",s.autoDetectIntimacy,"Детекция секса"],["lc-sr",s.autoRollOnSex,"Авто-бросок"],["lc-sw",s.showStatusWidget,"Виджет статуса"],["lc-st",s.autoTimeProgress,"Авто-время"]];
    for (var ac = 0; ac < autoChecks.length; ac++) h.push('<label class="lc-checkbox"><input type="checkbox" id="' + autoChecks[ac][0] + '"' + (autoChecks[ac][1] ? ' checked' : '') + '><span>' + autoChecks[ac][2] + '</span></label>');
    h.push('</div>');
    h.push('<div class="lc-section"><h4>Промпт</h4>');
    h.push('<label class="lc-checkbox"><input type="checkbox" id="lc-pon"' + (s.promptInjectionEnabled ? ' checked' : '') + '><span>Инъекция в промпт</span></label>');
    h.push('<div class="lc-row"><label>Позиция</label><select class="lc-select" id="lc-ppos"><option value="authornote"' + (s.promptInjectionPosition === "authornote" ? " selected" : "") + '>Author Note</option><option value="system"' + (s.promptInjectionPosition === "system" ? " selected" : "") + '>System</option></select></div>');
    h.push('<div class="lc-row"><label>AU пресет</label><select class="lc-select" id="lc-aup"><option value="realism"' + (s.auPreset === "realism" ? " selected" : "") + '>Реализм</option><option value="omegaverse"' + (s.auPreset === "omegaverse" ? " selected" : "") + '>Омегаверс</option><option value="fantasy"' + (s.auPreset === "fantasy" ? " selected" : "") + '>Фэнтези</option></select></div></div>');
    h.push('<div class="lc-section"><h4>Дата мира</h4><div class="lc-row"><input type="number" class="lc-input" id="lc-dy" value="' + s.worldDate.year + '" style="width:65px"><input type="number" class="lc-input" id="lc-dm" value="' + s.worldDate.month + '" min="1" max="12" style="width:42px"><input type="number" class="lc-input" id="lc-dd" value="' + s.worldDate.day + '" min="1" max="31" style="width:42px"><input type="number" class="lc-input" id="lc-dh" value="' + s.worldDate.hour + '" min="0" max="23" style="width:42px"><button class="lc-btn lc-btn-sm" id="lc-da">OK</button></div><div class="lc-btn-group" style="margin-top:4px"><button class="lc-btn lc-btn-sm" id="lc-d1">+1 день</button><button class="lc-btn lc-btn-sm" id="lc-d7">+7 дней</button><label class="lc-checkbox"><input type="checkbox" id="lc-df"' + (s.worldDate.frozen ? ' checked' : '') + '><span>❄️ Заморозка</span></label></div></div>');
    h.push('<div class="lc-section"><h4>Данные</h4><div class="lc-btn-group"><button class="lc-btn lc-btn-sm" id="lc-exp">📤 Экспорт</button><button class="lc-btn lc-btn-sm" id="lc-imp">📥 Импорт</button><button class="lc-btn lc-btn-sm lc-btn-danger" id="lc-rst">🗑️ Сброс</button></div></div>');
    h.push('</div>'); // settings tab
    h.push('</div></div>'); // body, panel
    return h.join("");
}

// ========================
// RENDER AU SETTINGS (NEW)
// ========================
function renderAuSettings() {
    var el = document.getElementById("lc-au-panel");
    if (!el) return;
    var s = S();
    var preset = s.auPreset;
    var h = [];

    h.push('<div class="lc-section"><h4>🌐 Текущий пресет: <strong>' + ({ realism: "Реализм", omegaverse: "Омегаверс", fantasy: "Фэнтези" }[preset] || preset) + '</strong></h4>');
    if (!s.modules.auOverlay && preset !== "realism") h.push('<div class="lc-info-row" style="color:orange">⚠️ AU Оверлей выключен в настройках!</div>');
    h.push('</div>');

    if (preset === "omegaverse") {
        var ov = s.auSettings.omegaverse;
        // Heat
        h.push('<div class="lc-section"><h4>🔥 Течка (Heat)</h4>');
        h.push('<div class="lc-editor-grid">');
        h.push('<div class="lc-editor-field"><label>Цикл (дни)</label><input type="number" class="lc-input lc-au-inp" data-path="omegaverse.heatCycleLength" min="7" max="120" value="' + ov.heatCycleLength + '"></div>');
        h.push('<div class="lc-editor-field"><label>Длительность (дни)</label><input type="number" class="lc-input lc-au-inp" data-path="omegaverse.heatDuration" min="1" max="30" value="' + ov.heatDuration + '"></div>');
        h.push('<div class="lc-editor-field"><label>Предтечка (дни)</label><input type="number" class="lc-input lc-au-inp" data-path="omegaverse.preHeatDays" min="0" max="7" value="' + ov.preHeatDays + '"></div>');
        h.push('<div class="lc-editor-field"><label>Посттечка (дни)</label><input type="number" class="lc-input lc-au-inp" data-path="omegaverse.postHeatDays" min="0" max="7" value="' + ov.postHeatDays + '"></div>');
        h.push('<div class="lc-editor-field"><label>Бонус фертильности</label><input type="number" class="lc-input lc-au-inp" data-path="omegaverse.heatFertilityBonus" min="0" max="0.95" step="0.05" value="' + ov.heatFertilityBonus + '"></div>');
        h.push('<div class="lc-editor-field"><label>Интенсивность</label><select class="lc-select lc-au-sel" data-path="omegaverse.heatIntensity"><option value="mild"' + (ov.heatIntensity === "mild" ? " selected" : "") + '>Слабая</option><option value="moderate"' + (ov.heatIntensity === "moderate" ? " selected" : "") + '>Средняя</option><option value="severe"' + (ov.heatIntensity === "severe" ? " selected" : "") + '>Сильная</option></select></div>');
        h.push('</div></div>');

        // Rut
        h.push('<div class="lc-section"><h4>💢 Гон (Rut)</h4>');
        h.push('<div class="lc-editor-grid">');
        h.push('<div class="lc-editor-field"><label>Цикл (дни)</label><input type="number" class="lc-input lc-au-inp" data-path="omegaverse.rutCycleLength" min="7" max="120" value="' + ov.rutCycleLength + '"></div>');
        h.push('<div class="lc-editor-field"><label>Длительность (дни)</label><input type="number" class="lc-input lc-au-inp" data-path="omegaverse.rutDuration" min="1" max="30" value="' + ov.rutDuration + '"></div>');
        h.push('<div class="lc-editor-field"><label>Предгон (дни)</label><input type="number" class="lc-input lc-au-inp" data-path="omegaverse.preRutDays" min="0" max="7" value="' + ov.preRutDays + '"></div>');
        h.push('<div class="lc-editor-field"><label>Постгон (дни)</label><input type="number" class="lc-input lc-au-inp" data-path="omegaverse.postRutDays" min="0" max="7" value="' + ov.postRutDays + '"></div>');
        h.push('<div class="lc-editor-field"><label>Интенсивность</label><select class="lc-select lc-au-sel" data-path="omegaverse.rutIntensity"><option value="mild"' + (ov.rutIntensity === "mild" ? " selected" : "") + '>Слабая</option><option value="moderate"' + (ov.rutIntensity === "moderate" ? " selected" : "") + '>Средняя</option><option value="severe"' + (ov.rutIntensity === "severe" ? " selected" : "") + '>Сильная</option></select></div>');
        h.push('</div></div>');
        ...
        h.push('<div class="lc-editor-field"><label>Длительность узла (мин)</label><input type="number" class="lc-input lc-au-inp" data-path="omegaverse.knotDurationMin" min="5" max="120" value="' + ov.knotDurationMin + '"></div>');
        h.push('</div>');

        // Bonding
        h.push('<div class="lc-section"><h4>💞 Связь (Bonding)</h4>');
        h.push('<label class="lc-checkbox"><input type="checkbox" class="lc-au-chk" data-path="omegaverse.bondingEnabled"' + (ov.bondingEnabled ? ' checked' : '') + '><span>Связь включена</span></label>');
        h.push('<div class="lc-editor-grid">');
        ...
        h.push('<div class="lc-editor-field"><label>Дни ломки</label><input type="number" class="lc-input lc-au-inp" data-path="omegaverse.bondWithdrawalDays" min="1" max="60" value="' + ov.bondWithdrawalDays + '"></div>');
        h.push('</div>');
        ...
        h.push('<label class="lc-checkbox"><input type="checkbox" class="lc-au-chk" data-path="omegaverse.bondEffectProximity"' + (ov.bondEffectProximity ? ' checked' : '') + '><span>Тяга к близости</span></label>');
        ...
        // Bonding actions
        h.push('<div class="lc-row" style="margin-top:8px;flex-wrap:wrap"><select class="lc-select lc-char-select" id="lc-bond-c1">' + charOptions() + '</select><span>⟷</span><select class="lc-select lc-char-select" id="lc-bond-c2">' + charOptions() + '</select><button class="lc-btn lc-btn-sm lc-btn-success" id="lc-bond-create">💞 Создать связь</button><button class="lc-btn lc-btn-sm lc-btn-danger" id="lc-bond-break">💔 Разорвать</button></div>');
        h.push('</div>');
        ...
        h.push('<div class="lc-editor-field"><label>Эффективность</label><input type="number" class="lc-input lc-au-inp" data-path="omegaverse.suppressantEffectiveness" min="0" max="1" step="0.05" value="' + ov.suppressantEffectiveness + '"></div>');
        h.push('<label class="lc-checkbox"><input type="checkbox" class="lc-au-chk" data-path="omegaverse.suppressantSideEffects"' + (ov.suppressantSideEffects ? ' checked' : '') + '><span>Побочные эффекты</span></label>');
        h.push('</div>');

        // Physiology
        h.push('<div class="lc-section"><h4>🧬 Физиология</h4>');
        h.push('<label class="lc-checkbox"><input type="checkbox" class="lc-au-chk" data-path="omegaverse.slickEnabled"' + (ov.slickEnabled ? ' checked' : '') + '><span>💧 Самосмазка (омега)</span></label>');
        ...
        h.push('<label class="lc-checkbox"><input type="checkbox" class="lc-au-chk" data-path="omegaverse.purringEnabled"' + (ov.purringEnabled ? ' checked' : '') + '><span>😺 Мурлыканье</span></label>');
        h.push('</div>');

        // Pregnancy
        h.push('<div class="lc-section"><h4>🤰 Беременность</h4>');
        h.push('<label class="lc-checkbox"><input type="checkbox" class="lc-au-chk" data-path="omegaverse.maleOmegaPregnancy"' + (ov.maleOmegaPregnancy ? ' checked' : '') + '><span>М-омега может забеременеть</span></label>');
        h.push('<div class="lc-editor-field"><label>Недель беременности</label><input type="number" class="lc-input lc-au-inp" data-path="omegaverse.pregnancyWeeks" min="4" max="60" value="' + ov.pregnancyWeeks + '"></div>');
        h.push('<div class="lc-editor-field"><label>Шанс двойни</label><input type="number" class="lc-input lc-au-inp" data-path="omegaverse.twinChance" min="0" max="1" step="0.05" value="' + ov.twinChance + '"></div>');
        h.push('</div>');

        // Hierarchy
        h.push('<div class="lc-section"><h4>👑 Иерархия</h4>');
        h.push('<label class="lc-checkbox"><input type="checkbox" class="lc-au-chk" data-path="omegaverse.alphaCommandVoice"' + (ov.alphaCommandVoice ? ' checked' : '') + '><span>Командный голос альфы</span></label>');
        h.push('<label class="lc-checkbox"><input type="checkbox" class="lc-au-chk" data-path="omegaverse.omegaSubmission"' + (ov.omegaSubmission ? ' checked' : '') + '><span>Подчинение омеги</span></label>');
        h.push('</div>');

    } else if (preset === "fantasy") {
        var fan = s.auSettings.fantasy;
        h.push('<div class="lc-section"><h4>🧝 Беременность по расам (недели)</h4><div class="lc-editor-grid">');
        var raceNames = Object.keys(fan.pregnancyByRace);
        var raceLabels = { human: "Человек", elf: "Эльф", dwarf: "Гном", orc: "Орк", demon: "Демон", vampire: "Вампир", werewolf: "Оборотень", fairy: "Фея", dragon: "Дракон", halfling: "Полурослик" };
        for (var ri = 0; ri < raceNames.length; ri++) {
            var rn = raceNames[ri];
            h.push('<div class="lc-editor-field"><label>' + (raceLabels[rn] || rn) + '</label><input type="number" class="lc-input lc-au-race" data-race="' + rn + '" min="4" max="120" value="' + fan.pregnancyByRace[rn] + '"></div>');
        }
        h.push('</div></div>');
        h.push('<div class="lc-section"><h4>✨ Магия</h4>');
        ...
        h.push('<label class="lc-checkbox"><input type="checkbox" class="lc-au-chk" data-path="fantasy.acceleratedPregnancy"' + (fan.acceleratedPregnancy ? ' checked' : '') + '><span>Ускоренная беременность</span></label>');
        h.push('<div class="lc-editor-field"><label>Множитель ускорения</label><input type="number" class="lc-input lc-au-inp" data-path="fantasy.accelerationFactor" min="0.1" max="10" step="0.1" value="' + fan.accelerationFactor + '"></div>');
        h.push('</div>');

    } else if (preset === "realism") {
        h.push('<div class="lc-section"><div class="lc-info-row">В режиме реализма AU-настройки не применяются. Выберите другой пресет в настройках.</div></div>');
    }

    // Oviposition (always shown if enabled)
    if (s.auSettings.oviposition.enabled) {
        ...
        h.push('<div class="lc-editor-field"><label>Мин. яиц</label><input type="number" class="lc-input lc-au-inp" data-path="oviposition.eggCountMin" min="1" max="20" value="' + ovi.eggCountMin + '"></div>');
        h.push('<div class="lc-editor-field"><label>Макс. яиц</label><input type="number" class="lc-input lc-au-inp" data-path="oviposition.eggCountMax" min="1" max="50" value="' + ovi.eggCountMax + '"></div>');
        h.push('<div class="lc-editor-field"><label>Вынашивание (дни)</label><input type="number" class="lc-input lc-au-inp" data-path="oviposition.gestationDays" min="1" max="90" value="' + ovi.gestationDays + '"></div>');
        h.push('<div class="lc-editor-field"><label>Откладывание (дни)</label><input type="number" class="lc-input lc-au-inp" data-path="oviposition.layingDuration" min="1" max="14" value="' + ovi.layingDuration + '"></div>');
        h.push('<div class="lc-editor-field"><label>Инкубация (дни)</label><input type="number" class="lc-input lc-au-inp" data-path="oviposition.incubationDays" min="1" max="90" value="' + ovi.incubationDays + '"></div>');
        h.push('<div class="lc-editor-field"><label>Шанс оплодотв.</label><input type="number" class="lc-input lc-au-inp" data-path="oviposition.fertilizationChance" min="0" max="1" step="0.05" value="' + ovi.fertilizationChance + '"></div>');
        h.push('<div class="lc-editor-field"><label>Скорлупа</label><select class="lc-select lc-au-sel" data-path="oviposition.shellType"><option value="hard"' + (ovi.shellType === "hard" ? " selected" : "") + '>Твёрдая</option><option value="soft"' + (ovi.shellType === "soft" ? " selected" : "") + '>Мягкая</option><option value="membrane"' + (ovi.shellType === "membrane" ? " selected" : "") + '>Мембрана</option></select></div>');
        h.push('<div class="lc-editor-field"><label>Размер яиц</label><select class="lc-select lc-au-sel" data-path="oviposition.eggSize"><option value="small"' + (ovi.eggSize === "small" ? " selected" : "") + '>Малые</option><option value="medium"' + (ovi.eggSize === "medium" ? " selected" : "") + '>Средние</option><option value="large"' + (ovi.eggSize === "large" ? " selected" : "") + '>Крупные</option></select></div>');
        h.push('<div class="lc-editor-field"><label>Боль</label><select class="lc-select lc-au-sel" data-path="oviposition.painLevel"><option value="mild"' + (ovi.painLevel === "mild" ? " selected" : "") + '>Слабая</option><option value="moderate"' + (ovi.painLevel === "moderate" ? " selected" : "") + '>Средняя</option><option value="severe"' + (ovi.painLevel === "severe" ? " selected" : "") + '>Сильная</option></select></div>');
        h.push('<div class="lc-editor-field"><label>Уход после (дни)</label><input type="number" class="lc-input lc-au-inp" data-path="oviposition.aftercareDays" min="0" max="14" value="' + ovi.aftercareDays + '"></div>');
        h.push('</div></div>');
    }
...
function bindAuInputs() {
    ...
    if (bondCreate) bondCreate.addEventListener("click", function () {
        ...
        var p1 = s.characters[c1.value]; var p2 = s.characters[c2.value];
        if (!p1 || !p2) return;
        var bm1 = new BondManager(p1);
        if (!bm1.canBond()) { toastr.warning(c1.value + " уже связан или бондинг отключён!"); return; }
        var bm2 = new BondManager(p2);
        if (!bm2.canBond()) { toastr.warning(c2.value + " уже связан!"); return; }
        bm1.createBond(c2.value);
        bm2.createBond(c1.value);
        toastr.success("💞 Связь создана: " + c1.value + " ⟷ " + c2.value);
        rebuild();
    });
    var bondBreak = document.getElementById("lc-bond-break");
    if (bondBreak) bondBreak.addEventListener("click", function () {
        var c1 = document.getElementById("lc-bond-c1");
        if (!c1) return;
        var p = s.characters[c1.value];
        if (!p || !p.bond || !p.bond.bonded) { toastr.warning("У этого персонажа нет связи!"); return; }
        if (!confirm("Разорвать связь " + c1.value + " ⟷ " + p.bond.partner + "?")) return;
        var bm = new BondManager(p);
        if (bm.breakBond()) { toastr.info("💔 Связь разорвана"); rebuild(); }
    });
}
...
function renderDashboard() {
    ...
    for (var i = 0; i < names.length; i++) {
        ...
        if (s.modules.cycle && p.cycle && p.cycle.enabled && !(p.pregnancy && p.pregnancy.active)) { var cm = new CycleManager(p); var ph = cm.phase(); tags.push(cm.emoji(ph) + cm.label(ph)); }
...
function renderCharList() {
    ...
    for (var i = 0; i < names.length; i++) {
        var n = names[i]; var p = s.characters[n]; var sexIcon = p.bioSex === "F" ? "\u2640" : "\u2642"; var secText = p.secondarySex ? " " + p.secondarySex : "";
        ...
        var bondBadge = (p.bond && p.bond.bonded) ? ' <span class="lc-tag">\uD83D\uDC9E' + p.bond.partner + '</span>' : "";
        html += '<div class="lc-char-card"><div class="lc-char-card-header"><span class="lc-char-card-name">' + sexIcon + ' ' + n + secText + '</span>' + srcBadge + eyeBadge + hairBadge + bondBadge + '</div><div class="lc-char-card-actions"><button class="lc-btn lc-btn-sm lc-edit-char" data-char="' + n + '">\u270F\uFE0F</button><button class="lc-btn lc-btn-sm lc-btn-danger lc-del-char" data-char="' + n + '">\u2715</button></div></div>';
    }
...
function renderHeatRut() {
    ...
    var p = s.characters[sel.value];
    if (!p || !s.modules.auOverlay || s.auPreset !== "omegaverse" || !p.secondarySex) { el.innerHTML = '<div class="lc-empty">AU не активен или нет 2-го пола</div>'; return; }
    ...
    if (p.secondarySex === "omega") {
        ...
        if (p.heat && p.heat.active) html += '<div class="lc-info-row">День: ' + p.heat.currentDay + ' / ' + p.heat.duration + '</div>';
        else html += '<div class="lc-info-row">До течки: ' + hr.heatDaysLeft() + ' дн.</div>';
        html += '<div class="lc-btn-group"><button class="lc-btn lc-btn-sm" id="lc-hr-th">\uD83D\uDD25 Запустить</button><button class="lc-btn lc-btn-sm" id="lc-hr-sh">\u23F9 Стоп</button><button class="lc-btn lc-btn-sm" id="lc-hr-su">\uD83D\uDC8A Супрессанты</button></div></div>';
    }
    if (p.secondarySex === "alpha") {
        var rPh = hr.rutPhase();
        html += '<div class="lc-section"><h4>\uD83D\uDCA2 ' + hr.rutLabel(rPh) + '</h4>';
        if (p.rut && p.rut.active) html += '<div class="lc-info-row">День: ' + p.rut.currentDay + ' / ' + p.rut.duration + '</div>';
        else html += '<div class="lc-info-row">До гона: ' + hr.rutDaysLeft() + ' дн.</div>';
        html += '<div class="lc-btn-group"><button class="lc-btn lc-btn-sm" id="lc-hr-tr">\uD83D\uDCA2 Запустить</button><button class="lc-btn lc-btn-sm" id="lc-hr-sr">\u23F9 Стоп</button></div></div>';
    }
    // Bond status
    if (p.bond) {
        var bm = new BondManager(p);
        html += '<div class="lc-section"><h4>💞 Связь</h4><div class="lc-info-row">' + bm.statusLabel() + '</div>';
        var eff = bm.effects(); if (eff.length > 0) html += '<div class="lc-info-row">Эффекты: ' + eff.join(", ") + '</div>';
        html += '</div>';
    }
    el.innerHTML = html;
...
function renderPregnancy() {
    ...
    var p = s.characters[sel.value]; if (!p || !p.pregnancy || !p.pregnancy.active) { el.innerHTML = '<div class="lc-empty">Нет беременности</div>'; return; }
    var pm = new PregManager(p); var pr = p.pregnancy; var progress = Math.round((pr.week / pr.maxWeeks) * 100);
    var html = '<div class="lc-preg-header"><span class="lc-preg-week">Неделя ' + pr.week + ' / ' + pr.maxWeeks + '</span><span class="lc-preg-trim">Триместр ' + pm.trimester() + '</span></div>';
    ...
    var sym = pm.symptoms(); html += '<div class="lc-info-row">Симптомы: ' + (sym.length > 0 ? sym.join(", ") : "нет") + '</div>';
    el.innerHTML = html;
}

function renderLabor() {
    var s = S(); var el = document.getElementById("lc-labor-panel"); var sel = document.getElementById("lc-labor-char"); if (!el || !sel) return;
    var p = s.characters[sel.value]; if (!p || !p.labor || !p.labor.active) { el.innerHTML = '<div class="lc-empty">Нет активных родов</div>'; return; }
    var lm = new LaborManager(p); var progress = Math.round((p.labor.dilation / 10) * 100);
    var html = '<div class="lc-labor-stage">' + LABOR_LABELS[p.labor.stage] + '</div><div class="lc-sw-mini-progress"><div class="lc-sw-mini-fill labor" style="width:' + progress + '%"></div></div>';
    html += '<div class="lc-info-row">Раскрытие: ' + p.labor.dilation + '/10 см | Время: ' + p.labor.hoursElapsed + ' ч</div><div class="lc-labor-desc">' + lm.description() + '</div>';
    el.innerHTML = html;
}
...
function renderBabies() {
    ...
    for (var i = 0; i < p.babies.length; i++) {
        var b = p.babies[i]; var bm = new BabyManager(b); var sexIcon = b.sex === "M" ? "\u2642" : "\u2640";
        html += '<div class="lc-baby-card"><div class="lc-baby-header"><span class="lc-baby-name">' + sexIcon + ' ' + (b.name || "?") + '</span><span class="lc-baby-sex">' + bm.age() + '</span></div><div class="lc-baby-details">Мать: ' + b.mother + ' | Отец: ' + b.father + '</div><div class="lc-baby-actions"><button class="lc-btn lc-btn-sm lc-baby-edit" data-p="' + sel.value + '" data-i="' + i + '">\u270F\uFE0F</button><button class="lc-btn lc-btn-sm lc-btn-danger lc-baby-del" data-p="' + sel.value + '" data-i="' + i + '">\u2715</button></div></div>';
    }
    el.innerHTML = html;
}
...
function renderOvi() {
    ...
    var html = '<div class="lc-ovi-phase">' + (OVI_PHASES[p.oviposition.phase] || p.oviposition.phase) + '</div><div class="lc-sw-mini-progress"><div class="lc-sw-mini-fill ovi" style="width:' + prog + '%"></div></div>';
...
function renderRelations() {
    ...
    if (rels.length === 0) { el.innerHTML = '<div class="lc-empty">Нет связей</div>'; return; }
    var html = "";
    for (var i = 0; i < rels.length; i++) { var r = rels[i]; html += '<div class="lc-dice-entry">' + r.char1 + ' \u2192 ' + r.char2 + ': <strong>' + r.type + '</strong>' + (r.notes ? ' <span style="color:#666">(' + r.notes + ')</span>' : '') + ' <button class="lc-btn lc-btn-sm lc-btn-danger lc-del-rel" data-id="' + r.id + '">\u2715</button></div>'; }
    el.innerHTML = html;
    var delBtns = el.querySelectorAll(".lc-del-rel");
    for (var j = 0; j < delBtns.length; j++) delBtns[j].addEventListener("click", function () { Rels.remove(this.dataset.id); renderRelations(); });
}

function renderProfiles() {
    var s = S(); var curEl = document.getElementById("lc-prof-cur"); if (curEl) curEl.textContent = "Текущий: " + (s.currentChatId || "-") + " (" + Object.keys(s.characters).length + " перс.)";
    var el = document.getElementById("lc-prof-list"); if (!el) return; var list = Profiles.list();
    if (list.length === 0) { el.innerHTML = '<div class="lc-empty">Нет сохранённых профилей</div>'; return; }
    var html = "";
    for (var i = 0; i < list.length; i++) { var p = list[i]; html += '<div class="lc-profile-card' + (p.isCurrent ? ' current' : '') + '"><span>' + p.id.substring(0, 25) + ' (' + p.count + ' перс.)</span><div class="lc-btn-group"><button class="lc-btn lc-btn-sm lc-prof-load" data-id="' + p.id + '">\uD83D\uDCC2</button><button class="lc-btn lc-btn-sm lc-btn-danger lc-prof-del" data-id="' + p.id + '">\u2715</button></div></div>'; }
    el.innerHTML = html;
}

function renderDiceLog() {
    var el = document.getElementById("lc-dice-log"); if (!el) return; var logs = S().diceLog;
    if (!logs || logs.length === 0) { el.innerHTML = '<div class="lc-empty">Нет бросков</div>'; return; }
    var html = ""; var items = logs.slice().reverse().slice(0, 15);
    for (var i = 0; i < items.length; i++) { var e = items[i]; html += '<div class="lc-dice-entry ' + (e.result ? "lc-dice-success" : "lc-dice-fail") + '"><span class="lc-dice-ts">' + e.ts + '</span> ' + e.target + ': \uD83C\uDFB2' + e.roll + '/' + e.chance + '% ' + (e.result ? '\u2713' : '\u2717') + '</div>'; }
    el.innerHTML = html;
}

function renderIntimLog() {
    var el = document.getElementById("lc-intim-log"); if (!el) return; var logs = S().intimacyLog;
    if (!logs || logs.length === 0) { el.innerHTML = '<div class="lc-empty">Нет записей</div>'; return; }
    var html = ""; var items = logs.slice().reverse().slice(0, 15);
    for (var i = 0; i < items.length; i++) { var e = items[i]; html += '<div class="lc-intim-entry"><span class="lc-intim-ts">' + e.ts + '</span> ' + (e.parts || []).join(" + ") + ' ' + (e.type || "") + '</div>'; }
    el.innerHTML = html;
}

// ========================
// INJECT WIDGET (WAS MISSING!)
// ========================
function injectWidget(messageIndex) {
    ...
    // Find the message element and append widget
    try {
        var msgEl = document.querySelector('#chat .mes[mesid="' + messageIndex + '"]');
        if (!msgEl) return;
        var existing = msgEl.querySelector(".lc-status-widget");
        if (existing) existing.remove();
        var mesText = msgEl.querySelector(".mes_text");
        if (mesText) mesText.insertAdjacentHTML("afterend", widgetHtml);
    } catch (e) {
        console.warn("[BunnyCycle] Widget injection failed:", e.message);
    }
}
...
function generateWidget() {
    ...
    for (var check = 0; check < charEntries.length; check++) {
        ...
        if (cp.heat && cp.heat.active) hasContent = true; if (cp.rut && cp.rut.active) hasContent = true;
    ...
    var h = [];
    h.push('<div class="lc-status-widget"><div class="lc-sw-header" onclick="var body=this.nextElementSibling;var arrow=this.querySelector(\'.lc-sw-arrow\');if(body.style.display===\'none\'){body.style.display=\'block\';arrow.textContent=\'\\u25BC\';}else{body.style.display=\'none\';arrow.textContent=\'\\u25B6\';}"><span>🐰 BunnyCycle</span><span class="lc-sw-arrow">▼</span></div><div class="lc-sw-body"><div class="lc-sw-date">' + formatDate(s.worldDate) + '</div>');
    for (var i = 0; i < charEntries.length; i++) {
        var name = charEntries[i]; var p = s.characters[name]; var sexIcon = p.bioSex === "F" ? "\u2640" : "\u2642"; var hasAnyBlock = false; var charHtml = [];
        // Compact widget blocks
        if (s.modules.labor && p.labor && p.labor.active) { hasAnyBlock = true; var lm = new LaborManager(p); charHtml.push('<div class="lc-sw-detail-block">🏥 ' + LABOR_LABELS[p.labor.stage] + ' | ' + p.labor.dilation + '/10 см</div>'); }
        if (s.modules.pregnancy && p.pregnancy && p.pregnancy.active && !(p.labor && p.labor.active)) { hasAnyBlock = true; var pm = new PregManager(p); charHtml.push('<div class="lc-sw-detail-block">🤰 Нед.' + p.pregnancy.week + '/' + p.pregnancy.maxWeeks + ' | ' + pm.size() + ' | ' + pm.movements() + '</div>'); }
        if (p.heat && p.heat.active) { hasAnyBlock = true; var hr = new HeatRutManager(p); charHtml.push('<div class="lc-sw-detail-block">🔥 ' + hr.heatLabel(hr.heatPhase()) + ' Д' + p.heat.currentDay + '/' + p.heat.duration + '</div>'); }
        if (p.rut && p.rut.active) { hasAnyBlock = true; var hrr = new HeatRutManager(p); charHtml.push('<div class="lc-sw-detail-block">💢 ' + hrr.rutLabel(hrr.rutPhase()) + ' Д' + p.rut.currentDay + '/' + p.rut.duration + '</div>'); }
        if (p.bond && p.bond.bonded) { hasAnyBlock = true; charHtml.push('<div class="lc-sw-detail-block">💞 Связь: ' + p.bond.partner + ' (' + p.bond.strength + '%)</div>'); }
        if (p.bond && p.bond.withdrawalActive) { hasAnyBlock = true; charHtml.push('<div class="lc-sw-detail-block">💔 Ломка: день ' + p.bond.daysSinceSeparation + '</div>'); }
        if (s.modules.cycle && p.cycle && p.cycle.enabled && !(p.pregnancy && p.pregnancy.active) && !(p.labor && p.labor.active)) { hasAnyBlock = true; var cm = new CycleManager(p); charHtml.push('<div class="lc-sw-detail-block">' + cm.emoji(cm.phase()) + ' ' + cm.label(cm.phase()) + ' Д' + p.cycle.currentDay + ' Ферт:' + Math.round(cm.fertility() * 100) + '%</div>'); }
        if (p.oviposition && p.oviposition.active) { hasAnyBlock = true; charHtml.push('<div class="lc-sw-detail-block">🥚 ' + (OVI_PHASES[p.oviposition.phase] || "") + ' (' + p.oviposition.eggCount + ' яиц)</div>'); }
        if (s.modules.baby && p.babies && p.babies.length > 0) { hasAnyBlock = true; charHtml.push('<div class="lc-sw-detail-block">👶 Детей: ' + p.babies.length + '</div>'); }
        if (hasAnyBlock) { h.push('<div class="lc-sw-char"><div class="lc-sw-char-name">' + sexIcon + ' ' + name + (p.secondarySex ? ' <span class="lc-sw-sec-badge">' + p.secondarySex + '</span>' : '') + '</div>' + charHtml.join("") + '</div>'); }
    }
    h.push('</div></div>');
    return h.join("");
}
...
function showDicePopup(result, targetName, isAuto) {
    var oldOverlay = document.querySelector(".lc-overlay"); var oldPopup = document.querySelector(".lc-popup"); if (oldOverlay) oldOverlay.remove(); if (oldPopup) oldPopup.remove();
    ...
    var html = '<div class="lc-popup-title">\uD83C\uDFB2 Бросок на зачатие</div>' + (isAuto ? '<div class="lc-popup-auto">Авто-детекция</div>' : '') + '<div class="lc-popup-details"><strong>' + targetName + '</strong> | Шанс: ' + result.chance + '%</div><div class="lc-popup-result ' + (result.result ? "success" : "fail") + '">' + result.roll + ' / ' + result.chance + '</div><div class="lc-popup-verdict ' + (result.result ? "success" : "fail") + '">' + (result.result ? '\u2713 ЗАЧАТИЕ!' : '\u2717 Не произошло') + '</div><div class="lc-popup-actions"><button class="lc-btn lc-btn-success" id="lc-dp-ok">\u2713 OK</button><button class="lc-btn" id="lc-dp-re">\uD83C\uDFB2 Переброс</button><button class="lc-btn lc-btn-danger" id="lc-dp-no">\u2715 Отмена</button></div>';
    popup.innerHTML = html; document.body.appendChild(overlay); document.body.appendChild(popup);
    document.getElementById("lc-dp-ok").addEventListener("click", function () { if (result.result) { var p = S().characters[targetName]; if (p && canGetPregnant(p)) { var fatherName = (result.parts || []).find(function (x) { return x !== targetName; }) || "?"; new PregManager(p).start(fatherName, 1); saveSettingsDebounced(); rebuild(); } } overlay.remove(); popup.remove(); });
    document.getElementById("lc-dp-re").addEventListener("click", function () { overlay.remove(); popup.remove(); var newResult = Intimacy.roll(targetName, { parts: result.parts, type: result.type, ejac: result.ejac, auto: isAuto }); showDicePopup(newResult, targetName, isAuto); });
...
function showBabyForm(parentName, fatherName, existingBaby, babyIndex, isStandalone) {
    ...
    var html = '<div class="lc-popup-title">' + (isEdit ? '\u270F\uFE0F Редактирование' : '\uD83D\uDC76 Новый ребёнок') + '</div><div class="lc-editor-grid"><div class="lc-editor-field"><label>Имя</label><input class="lc-input" id="lc-bf-name" value="' + (baby.name || '') + '"></div><div class="lc-editor-field"><label>Пол</label><select class="lc-select" id="lc-bf-sex"><option value="random">\uD83C\uDFB2</option><option value="M"' + (baby.sex === "M" ? ' selected' : '') + '>\u2642</option><option value="F"' + (baby.sex === "F" ? ' selected' : '') + '>\u2640</option></select></div><div class="lc-editor-field"><label>Глаза</label><input class="lc-input" id="lc-bf-eyes" value="' + (baby.eyeColor || '') + '"></div><div class="lc-editor-field"><label>Волосы</label><input class="lc-input" id="lc-bf-hair" value="' + (baby.hairColor || '') + '"></div>';
    ...
    html += '</div><div class="lc-popup-actions"><button class="lc-btn lc-btn-success" id="lc-bf-save">\uD83D\uDCBE Сохранить</button><button class="lc-btn" id="lc-bf-cancel">Отмена</button></div>';
    ...
    document.getElementById("lc-bf-save").addEventListener("click", function () {
        ...
        if (isEdit) { var editBaby = s.characters[parentName] && s.characters[parentName].babies && s.characters[parentName].babies[babyIndex]; if (editBaby) { editBaby.name = name; editBaby.sex = sex; if (eyes) editBaby.eyeColor = eyes; if (hair) editBaby.hairColor = hair; var ageInput = document.getElementById("lc-bf-age"); if (ageInput) { editBaby.ageDays = parseInt(ageInput.value) || 0; new BabyManager(editBaby).update(); } saveSettingsDebounced(); rebuild(); } }
        else if (isStandalone) { var moVal = document.getElementById("lc-bf-mo") ? document.getElementById("lc-bf-mo").value : "?"; var faVal = document.getElementById("lc-bf-fa") ? document.getElementById("lc-bf-fa").value : "?"; var toVal = document.getElementById("lc-bf-to") ? document.getElementById("lc-bf-to").value : null; if (toVal && s.characters[toVal]) { var newBaby = BabyManager.generate(s.characters[moVal], faVal, { name: name, sex: sex, eyeColor: eyes, hairColor: hair }); newBaby.mother = moVal; newBaby.father = faVal; s.characters[toVal].babies.push(newBaby); Rels.addBirth(moVal, faVal, name); saveSettingsDebounced(); rebuild(); } }
        else { var mother = s.characters[parentName]; if (mother) { var newBaby2 = BabyManager.generate(mother, fatherName, { name: name, sex: sex, eyeColor: eyes, hairColor: hair }); mother.babies.push(newBaby2); Rels.addBirth(parentName, fatherName, name); var lm = new LaborManager(mother); lm.deliver(); if (lm.l.babiesDelivered >= lm.l.totalBabies) lm.end(); saveSettingsDebounced(); rebuild(); } }
...
function openEditor(name) {
    ...
    var editorEl = document.getElementById("lc-char-editor"); if (editorEl) editorEl.classList.remove("hidden");
...
function closeEditor() { currentEditName = null; var editorEl = document.getElementById("lc-char-editor"); if (editorEl) editorEl.classList.add("hidden"); }
function saveEditor() {
...
function bindAll() {
    ...
    if (headerEl) headerEl.addEventListener("click", function (e) { if (e.target.closest(".lc-switch")) return; s.panelCollapsed = !s.panelCollapsed; var panelEl = document.getElementById("bunnycycle-panel"); if (panelEl) panelEl.classList.toggle("collapsed", s.panelCollapsed); var arrowEl = this.querySelector(".lc-collapse-arrow"); if (arrowEl) arrowEl.innerHTML = s.panelCollapsed ? "▶" : "▼"; saveSettingsDebounced(); });
    var enabledEl = document.getElementById("lc-enabled"); if (enabledEl) enabledEl.addEventListener("change", function () { s.enabled = this.checked; saveSettingsDebounced(); });
    // Tabs
    var tabButtons = document.querySelectorAll(".lifecycle-tab");
    for (var ti = 0; ti < tabButtons.length; ti++) tabButtons[ti].addEventListener("click", function () { var allTabs = document.querySelectorAll(".lifecycle-tab"); for (var j = 0; j < allTabs.length; j++) allTabs[j].classList.remove("active"); var allPanels = document.querySelectorAll(".lifecycle-tab-content"); for (var k = 0; k < allPanels.length; k++) allPanels[k].classList.remove("active"); this.classList.add("active"); var targetPanel = document.querySelector('.lifecycle-tab-content[data-tab="' + this.dataset.tab + '"]'); if (targetPanel) targetPanel.classList.add("active"); rebuild(); });
    // Sync/Add/Reparse
    ...
    var reparseBtn = document.getElementById("lc-reparse"); if (reparseBtn) reparseBtn.addEventListener("click", async function () { CharAnalyzer.clearCache(); ChatAnalyzer.clearCache(); var names = Object.keys(s.characters); for (var i = 0; i < names.length; i++) { var p = s.characters[names[i]]; p._mB = false; p._mE = false; p._mH = false; p._mR = false; p._mS = false; p._sexConfidence = 0; } toastr.info("AI анализирует..."); await syncChars(); rebuild(); toastr.success("AI-скан завершён!"); });
    // Char list
    var charList = document.getElementById("lc-char-list"); if (charList) charList.addEventListener("click", function (e) { var editBtn = e.target.closest(".lc-edit-char"); var delBtn = e.target.closest(".lc-del-char"); if (editBtn) openEditor(editBtn.dataset.char); if (delBtn && confirm("Удалить персонажа?")) { delete s.characters[delBtn.dataset.char]; saveSettingsDebounced(); rebuild(); } });
    // Editor
    ...
    // Relations
    var relAdd = document.getElementById("lc-rel-add"); if (relAdd) relAdd.addEventListener("click", function () { var c1 = document.getElementById("lc-rel-c1"); var c2 = document.getElementById("lc-rel-c2"); var tp = document.getElementById("lc-rel-tp"); var notes = document.getElementById("lc-rel-n"); if (!c1 || !c2 || !tp || c1.value === c2.value) return; Rels.add(c1.value, c2.value, tp.value, notes ? notes.value : ""); if (notes) notes.value = ""; renderRelations(); });
    // Cycle
    var cycChar = document.getElementById("lc-cyc-char"); if (cycChar) cycChar.addEventListener("change", renderCycle);
    var cycSetDay = document.getElementById("lc-cyc-setday"); if (cycSetDay) cycSetDay.addEventListener("click", function () { var selVal = document.getElementById("lc-cyc-char"); var dayInput = document.getElementById("lc-cyc-day"); if (!selVal || !dayInput) return; var p = s.characters[selVal.value]; if (!p || !p.cycle || !p.cycle.enabled) return; var d = parseInt(dayInput.value); if (d >= 1 && d <= p.cycle.length) { new CycleManager(p).setDay(d); saveSettingsDebounced(); renderCycle(); renderDashboard(); } });
    var phaseButtons = [["lc-cyc-mens","menstruation"],["lc-cyc-foll","follicular"],["lc-cyc-ovul","ovulation"],["lc-cyc-lut","luteal"]];
    for (var pi = 0; pi < phaseButtons.length; pi++) (function (btnId, phase) { var btn = document.getElementById(btnId); if (btn) btn.addEventListener("click", function () { var selVal = document.getElementById("lc-cyc-char"); if (!selVal) return; var p = s.characters[selVal.value]; if (!p || !p.cycle || !p.cycle.enabled) return; new CycleManager(p).setPhase(phase); saveSettingsDebounced(); renderCycle(); renderDashboard(); }); })(phaseButtons[pi][0], phaseButtons[pi][1]);
    var cycSkip = document.getElementById("lc-cyc-skip"); if (cycSkip) cycSkip.addEventListener("click", function () { var selVal = document.getElementById("lc-cyc-char"); if (!selVal) return; var p = s.characters[selVal.value]; if (!p || !p.cycle || !p.cycle.enabled) return; p.cycle.currentDay = 1; p.cycle.cycleCount++; saveSettingsDebounced(); renderCycle(); renderDashboard(); });
    // Heat/Rut
    var hrChar = document.getElementById("lc-hr-char"); if (hrChar) hrChar.addEventListener("change", renderHeatRut);
    // Intimacy
    var intLog = document.getElementById("lc-int-log"); if (intLog) intLog.addEventListener("click", function () { var t = document.getElementById("lc-int-t"); if (!t || !t.value) return; var pEl = document.getElementById("lc-int-p"); var tpEl = document.getElementById("lc-int-tp"); var ejEl = document.getElementById("lc-int-ej"); Intimacy.log({ parts: [t.value, pEl ? pEl.value : ""].filter(Boolean), type: tpEl ? tpEl.value : "vaginal", ejac: ejEl ? ejEl.value : "unknown" }); renderIntimLog(); });
    var intRoll = document.getElementById("lc-int-roll"); if (intRoll) intRoll.addEventListener("click", function () { var t = document.getElementById("lc-int-t"); if (!t || !t.value) return; var pEl = document.getElementById("lc-int-p"); var tpEl = document.getElementById("lc-int-tp"); var ejEl = document.getElementById("lc-int-ej"); var result = Intimacy.roll(t.value, { parts: [t.value, pEl ? pEl.value : ""].filter(Boolean), type: tpEl ? tpEl.value : "vaginal", ejac: ejEl ? ejEl.value : "unknown" }); if (result.reason === "not_eligible") { toastr.warning("Этот персонаж не может забеременеть!"); return; } showDicePopup(result, t.value, false); renderDiceLog(); });
    // Pregnancy
    var pregChar = document.getElementById("lc-preg-char"); if (pregChar) pregChar.addEventListener("change", renderPregnancy);
    var pregAdv = document.getElementById("lc-preg-adv"); if (pregAdv) pregAdv.addEventListener("click", function () { var sel = document.getElementById("lc-preg-char"); if (!sel) return; var p = s.characters[sel.value]; if (p && p.pregnancy && p.pregnancy.active) { new PregManager(p).advanceDay(7); saveSettingsDebounced(); renderPregnancy(); renderDashboard(); } });
    var pregSet = document.getElementById("lc-preg-set"); if (pregSet) pregSet.addEventListener("click", function () { var sel = document.getElementById("lc-preg-char"); if (!sel) return; var p = s.characters[sel.value]; if (!p || !p.pregnancy || !p.pregnancy.active) return; var w = prompt("Установить неделю:"); if (w) { p.pregnancy.week = clamp(parseInt(w), 1, p.pregnancy.maxWeeks); saveSettingsDebounced(); renderPregnancy(); } });
    var pregLabor = document.getElementById("lc-preg-labor"); if (pregLabor) pregLabor.addEventListener("click", function () { var sel = document.getElementById("lc-preg-char"); if (!sel) return; var p = s.characters[sel.value]; if (!p || !p.pregnancy || !p.pregnancy.active) return; new LaborManager(p).start(); saveSettingsDebounced(); renderLabor(); renderDashboard(); });
    var pregEnd = document.getElementById("lc-preg-end"); if (pregEnd) pregEnd.addEventListener("click", function () { var sel = document.getElementById("lc-preg-char"); if (!sel) return; var p = s.characters[sel.value]; if (!p || !p.pregnancy || !p.pregnancy.active || !confirm("Прервать беременность?")) return; p.pregnancy.active = false; if (p.cycle) p.cycle.enabled = true; saveSettingsDebounced(); renderPregnancy(); renderDashboard(); });
    // Labor
    var laborChar = document.getElementById("lc-labor-char"); if (laborChar) laborChar.addEventListener("change", renderLabor);
    var laborAdv = document.getElementById("lc-labor-adv"); if (laborAdv) laborAdv.addEventListener("click", function () { var sel = document.getElementById("lc-labor-char"); if (!sel) return; var p = s.characters[sel.value]; if (p && p.labor && p.labor.active) { new LaborManager(p).advance(); saveSettingsDebounced(); renderLabor(); } });
    var laborDeliver = document.getElementById("lc-labor-deliver"); if (laborDeliver) laborDeliver.addEventListener("click", function () { var sel = document.getElementById("lc-labor-char"); if (!sel) return; var p = s.characters[sel.value]; if (p && p.labor && p.labor.active) showBabyForm(sel.value, (p.pregnancy && p.pregnancy.father) || "?"); });
    var laborEnd = document.getElementById("lc-labor-end"); if (laborEnd) laborEnd.addEventListener("click", function () { var sel = document.getElementById("lc-labor-char"); if (!sel) return; var p = s.characters[sel.value]; if (p && p.labor && p.labor.active && confirm("Завершить роды?")) { new LaborManager(p).end(); saveSettingsDebounced(); renderLabor(); renderDashboard(); } });
    // Baby
    ...
    var babyList = document.getElementById("lc-baby-list"); if (babyList) babyList.addEventListener("click", function (e) { var editBtn = e.target.closest(".lc-baby-edit"); var delBtn = e.target.closest(".lc-baby-del"); if (editBtn) { var baby = s.characters[editBtn.dataset.p] && s.characters[editBtn.dataset.p].babies && s.characters[editBtn.dataset.p].babies[parseInt(editBtn.dataset.i)]; if (baby) showBabyForm(editBtn.dataset.p, baby.father, baby, parseInt(editBtn.dataset.i)); } if (delBtn && confirm("Удалить ребёнка?")) { if (s.characters[delBtn.dataset.p] && s.characters[delBtn.dataset.p].babies) { s.characters[delBtn.dataset.p].babies.splice(parseInt(delBtn.dataset.i), 1); saveSettingsDebounced(); renderBabies(); } } });
    // Ovi
    ...
    // Profiles
    var profSave = document.getElementById("lc-prof-save"); if (profSave) profSave.addEventListener("click", function () { Profiles.save(); renderProfiles(); toastr.success("Профиль сохранён!"); });
    ...
    var profList = document.getElementById("lc-prof-list"); if (profList) profList.addEventListener("click", function (e) { var loadBtn = e.target.closest(".lc-prof-load"); var delBtn = e.target.closest(".lc-prof-del"); if (loadBtn) { var pr = s.chatProfiles && s.chatProfiles[loadBtn.dataset.id]; if (pr) { s.characters = JSON.parse(JSON.stringify(pr.characters || {})); s.relationships = JSON.parse(JSON.stringify(pr.relationships || [])); s.worldDate = JSON.parse(JSON.stringify(pr.worldDate || DEFAULTS.worldDate)); s.currentChatId = loadBtn.dataset.id; saveSettingsDebounced(); rebuild(); toastr.success("Профиль загружен!"); } } if (delBtn && confirm("Удалить профиль?")) { Profiles.del(delBtn.dataset.id); renderProfiles(); } });
    // Settings: Modules
    ...
    for (var mi = 0; mi < modKeys.length; mi++) (function (id, key) { var el = document.getElementById(id); if (el) el.addEventListener("change", function () { s.modules[key] = this.checked; saveSettingsDebounced(); }); })(modKeys[mi], moduleBindings[modKeys[mi]]);
    ...
    // Settings: Automation
    var autoBindings = { "lc-sa": "autoSyncCharacters", "lc-sp": "autoParseCharInfo", "lc-sc": "parseFullChat", "lc-sd": "autoDetectIntimacy", "lc-sr": "autoRollOnSex", "lc-sw": "showStatusWidget", "lc-st": "autoTimeProgress" };
    var autoKeys = Object.keys(autoBindings);
    for (var ai = 0; ai < autoKeys.length; ai++) (function (id, key) { var el = document.getElementById(id); if (el) el.addEventListener("change", function () { s[key] = this.checked; saveSettingsDebounced(); }); })(autoKeys[ai], autoBindings[autoKeys[ai]]);
    // Settings: Prompt
    ...
    // Settings: Date
    var dateApply = document.getElementById("lc-da"); if (dateApply) dateApply.addEventListener("click", function () { s.worldDate.year = parseInt(document.getElementById("lc-dy").value) || 2025; s.worldDate.month = clamp(parseInt(document.getElementById("lc-dm").value) || 1, 1, 12); s.worldDate.day = clamp(parseInt(document.getElementById("lc-dd").value) || 1, 1, 31); s.worldDate.hour = clamp(parseInt(document.getElementById("lc-dh").value) || 12, 0, 23); saveSettingsDebounced(); renderDashboard(); });
    ...
    // Import
    var impBtn = document.getElementById("lc-imp"); if (impBtn) impBtn.addEventListener("click", function () { var input = document.createElement("input"); input.type = "file"; input.accept = ".json"; input.addEventListener("change", function (e) { var file = e.target.files[0]; if (!file) return; var reader = new FileReader(); reader.onload = function (ev) { try { extension_settings[EXT] = deepMerge(DEFAULTS, JSON.parse(ev.target.result)); saveSettingsDebounced(); var oldPanel = document.getElementById("bunnycycle-panel"); if (oldPanel) oldPanel.remove(); init(); toastr.success("Импортировано!"); } catch (err) { toastr.error("Ошибка JSON: " + err.message); } }; reader.readAsText(file); }); input.click(); });
    // Reset
    var rstBtn = document.getElementById("lc-rst"); if (rstBtn) rstBtn.addEventListener("click", function () { if (!confirm("Полный сброс всех данных BunnyCycle?")) return; extension_settings[EXT] = JSON.parse(JSON.stringify(DEFAULTS)); saveSettingsDebounced(); var oldPanel = document.getElementById("bunnycycle-panel"); if (oldPanel) oldPanel.remove(); init(); });
}
...
async function init() {
    try {
        console.log("[BunnyCycle] Initializing v1.1.0...");
        ...
        console.log("[BunnyCycle v1.1.0] Successfully loaded!");
    } catch (err) {
